...
export async function renderApprovalProcess(snapshot = null) {
    ...
    // KORREKTUR: Wir leeren den Bereich erst, wenn wir sicher sind, dass wir Daten haben.
    // approvalProcessArea.innerHTML = ''; // <-- Nicht hier leeren

    // KORREKTUR: Die gesamte Logik wird in einen try...catch-Block verpackt.
    try {
        if (!snapshot) {
            // Diese Zeile verursacht den Fehler, wenn der Index fehlt
            snapshot = await getDocs(query(roleChangeRequestsCollectionRef, orderBy('timestamp', 'desc')));
        }

        // JETZT leeren wir den Bereich
        ...
        approvalProcessArea.querySelectorAll('.withdraw-request-btn').forEach(button => button.addEventListener('click', async (e) => {
            ...
            // =================================================================
            // BEGINN DER KORREKTUR (Fehlende Klammer)
            // =================================================================
            } catch (error) { // <-- HIER WAR DIE KLAMMER VERGESSEN
    ...
    // KORREKTUR: Der Catch-Block fängt den Fehler ab (z.B. fehlender Index)
    } catch (error) {
        console.error("Fehler in renderApprovalProcess:", error);
        // KORREKTUR: Dem Benutzer eine hilfreiche Fehlermeldung anzeigen
        approvalProcessArea.innerHTML = `<div class="p-4 bg-red-100 text-red-700 rounded-lg">
            <p class="font-bold">Fehler beim Laden der Genehmigungen</p>
            <p class="text-sm mt-1">Die Daten konnten nicht geladen werden. Möglicherweise fehlt ein Datenbank-Index.</p>
            <p class="text-xs mt-2 font-mono">Fehler: ${error.message}</p>
            <p class="text-xs mt-1">Bitte prüfe die Browser-Konsole (F12) auf einen Link, um den Index zu erstellen.</p>
        </div>`;
    }
}