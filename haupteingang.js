...
import { initializeGeschenkemanagement, listenForGeschenke, ERINNERUNGEN } from './geschenkemanagement.js';
// // ENDE-ZIKA //
...
// ========================================
// STARTSEITEN-BENACHRICHTIGUNGEN (Erinnerungen)
// ========================================
window.updateHomeAlerts = function() {
    const alertBox = document.getElementById('global-app-alert-box');
    if (!alertBox) return;
    
    try {
        // Prüfe auf fällige Erinnerungen
        const now = new Date();
        const activeErinnerungen = Object.values(ERINNERUNGEN || {}).filter(e => {
            if (e.erledigt) return false;  // Erledigte ignorieren
            
            const erinnerungDatum = e.datum?.toDate ? e.datum.toDate() : new Date(e.datum);
            
            // Erinnerung ist fällig wenn Datum in der Vergangenheit oder heute
            return erinnerungDatum <= now;
        });
        
        if (activeErinnerungen.length > 0) {
            alertBox.innerHTML = `
                ...
                <span class="drop-shadow-sm">${activeErinnerungen.length} Erinnerung${activeErinnerungen.length > 1 ? 'en' : ''} fällig!</span>
            `;
            alertBox.className = 'mr-auto bg-orange-500 text-white px-4 py-2 rounded-xl animate-pulse font-bold text-sm cursor-pointer shadow-lg flex items-center gap-2 hover:bg-orange-600 transition transform hover:scale-105 border-2 border-white/20';
            alertBox.style.display = 'flex';
            alertBox.onclick = () => navigate('geschenkemanagementView');
        } else {
            alertBox.style.display = 'none';
        }
    } catch (e) {
        console.error("Fehler beim Aktualisieren der Home-Alerts:", e);
    }
};
