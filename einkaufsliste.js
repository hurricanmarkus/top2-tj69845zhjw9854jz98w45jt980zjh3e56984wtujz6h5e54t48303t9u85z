...
const MODES = [{ id: 'shop', label: 'Listenmodus' }, { id: 'manage', label: 'Wartung-Stammdaten' }, { id: 'notify', label: 'Erweiterte Einstellungen' }];
const MANAGE = [{ id: 'articles', label: 'Artikelwartung' }, { id: 'stores', label: 'Geschäftewartung' }, { id: 'categories', label: 'Kategorienwartung' }, { id: 'remarks', label: 'Anmerkungswartung' }, { id: 'notes', label: 'Notizwartung' }];
const ADVANCED = [{ id: 'general', label: 'Allgemein' }, { id: 'notification', label: 'Benachrichtigung' }, { id: 'sync', label: 'Synchronisierung' }, { id: 'settings', label: 'Einstellungen' }];
...
function normalizeVariantUnit(unit) {
    const value = String(unit || '').trim();
    return UNITS.includes(value) ? value : 'Stück';
}

function createEmptyVariant(seed = {}) {
    return {
        id: String(seed.id || makeLocalId('variant')),
        brand: String(seed.brand || '').trim(),
        label: String(seed.label || seed.name || '').trim(),
...
function isVariantArticle(article) {
    return String(article?.articleMode || article?.mode || '').trim() === 'variant';
}

function articleStoreIds(article) {
    if (!article) return [];
    if (!isVariantArticle(article)) return [...(article.storeIds || [])];
    return Array.from(new Set((article.variants || []).map((variant) => String(variant?.storeId || '').trim()).filter(Boolean)));
}
...
function normalizeArticle(article = {}) {
    ...
    const normalized = {
        ...
        defaultQuantity: Number(article.defaultQuantity || 1) || 1,
...
function createEmptyArticleEditor(mode = 'single') {
    return normalizeArticle({
        articleMode: mode === 'variant' ? 'variant' : 'single',
        title: '',
        aliases: [],
        defaultQuantity: 1,
        defaultUnit: 'Stück',
...
function cloneArticleEditor(article) {
    return normalizeArticle(JSON.parse(JSON.stringify(article || createEmptyArticleEditor())));
}

function findVariantById(article, variantId = '') {
    if (!article || !variantId) return null;
    return (article.variants || []).find((variant) => String(variant.id || '') === String(variantId || '')) || null;
}

function buildVariantGeneratedTitle(article, variant) {
    if (!variant) return String(article?.title || '').trim();
    ...
    if (brand && label) {
        ...
        return normalizedLabel.startsWith(`${normalizedBrand} `) ? label : `${brand} ${label}`.trim();
    }
    return label || articleTitle;
}

function buildVariantSearchTerms(article, variant) {
    const articleTitle = String(article?.title || '').trim();
    const aliases = article?.aliases || [];
    const label = String(variant?.label || '').trim();
    const brand = String(variant?.brand || '').trim();
    const quantity = formatEditableQty(variant?.quantity || '').replace(',', '.');
    const compactQuantity = quantity.replace('.', ',');
    const unit = String(variant?.unit || '').trim();
    const unitShort = unit === 'Gramm' ? 'g' : unit === 'Milliliter' ? 'ml' : unit === 'Liter' ? 'l' : unit === 'Kg' ? 'kg' : '';
    return Array.from(new Set([
        articleTitle,
        ...aliases,
        label,
        brand,
        buildVariantGeneratedTitle(article, variant),
        [brand, label, quantity, unit].filter(Boolean).join(' '),
        [label, quantity, unit].filter(Boolean).join(' '),
        unitShort && quantity ? `${quantity}${unitShort}` : '',
        unitShort && compactQuantity ? `${compactQuantity}${unitShort}` : ''
    ].map((entry) => String(entry || '').trim()).filter(Boolean)));
}
...
function resetInputSelection(clearTitle = false) {
    state.selectedArticleId = '';
    state.selectedVariantId = '';
    state.variantPickerArticleId = '';
...
function prefillInputFromArticle(article) {
    ...
    state.selectedArticleId = String(article.id || '');
    state.selectedVariantId = '';
    state.variantPickerArticleId = isVariantArticle(article) ? String(article.id || '') : '';
...
function prefillInputFromVariant(article, variant) {
    if (!article || !variant) return;
    const note = articlePrefillNote(article);
    state.selectedArticleId = String(article.id || '');
    state.selectedVariantId = String(variant.id || '');
    state.variantPickerArticleId = '';
    state.title = buildVariantGeneratedTitle(article, variant);
    state.q = formatEditableQty(variant.quantity || 1) || '1';
    state.unit = String(variant.unit || 'Stück');
    state.storeIds = variant.storeId ? [variant.storeId] : [];
    state.note = note;
    state.inputDetailsOpen = !!(state.storeIds.length || note);
}

function findExactArticleSelection(term) {
    const query = String(term || '').trim().toLowerCase();
    if (!query) return { article: null, variant: null };
    for (const article of state.articles) {
        const articleTerms = [article.title, ...(article.aliases || [])].map((entry) => String(entry || '').trim().toLowerCase()).filter(Boolean);
        if (articleTerms.includes(query)) return { article, variant: null };
        for (const variant of article.variants || []) {
            const terms = buildVariantSearchTerms(article, variant).map((entry) => String(entry || '').trim().toLowerCase());
            if (terms.includes(query)) return { article, variant };
        }
    }
...
function findArticleByCode(code = '') {
    ...
    if (!query) return { article: null, variant: null };
    for (const article of state.articles) {
        if ((article.eanCodes || []).includes(query)) return { article, variant: null };
        for (const variant of article.variants || []) {
            if ((variant.eanCodes || []).includes(query)) return { article, variant };
        }
    }
    return { article: null, variant: null };
}
...
function suggestionScore(query, text) {
    ...
    return {
        startsWith: value.startsWith(query) ? 0 : 1,
        position: value.indexOf(query),
        lengthDelta: Math.abs(value.length - query.length)
    };
}

function renderVariantPicker() {
    if (!state.variantPickerArticleId || state.listMode !== 'input') return '';
    const article = state.articles.find((entry) => entry.id === state.variantPickerArticleId);
    if (!article || !isVariantArticle(article) || !(article.variants || []).length) return '';
    return `<div class="elc border border-amber-200 bg-amber-50 space-y-2"><div class="flex flex-wrap items-center justify-between gap-2"><div><div class="text-sm font-black text-amber-900">Varianten auswählen</div><div class="text-xs text-amber-800">${escapeHtml(article.title || 'Variantenartikel')} hat ${article.variants.length} Variante(n).</div></div><button class="elb bg-white text-amber-800" data-a="close-variant-picker">Schließen</button></div><div class="space-y-2">${article.variants.map((variant) => `<button type="button" class="w-full rounded-2xl border border-amber-200 bg-white px-3 py-2 text-left" data-a="pick-variant-choice" data-id="${article.id}" data-variant="${variant.id}"><div class="flex flex-wrap items-center gap-2"><div class="min-w-0 flex-1"><div class="truncate text-sm font-black text-slate-900">${escapeHtml(buildVariantGeneratedTitle(article, variant))}</div><div class="truncate text-[11px] text-slate-500">${escapeHtml(state.stores.find((store) => store.id === variant.storeId)?.name || 'Ohne Geschäft')} · ${fmtQty(variant.quantity || 1)} ${escapeHtml(variant.unit || 'Stück')}</div></div>${state.selectedVariantId === variant.id ? chip('Ausgewählt', 'bg-emerald-100 text-emerald-700') : ''}</div></button>`).join('')}</div></div>`;
}
...
function ensureRoot() {
    ...
    root.addEventListener('focusin', onFocusInActive);
...
function onFocusInActive(e) {
    const target = e.target;
    if (!target || target.id !== 'el-q') return;
    selectInputContents(target);
}
...
async function saveNotificationTemplate() {
    ...
    if (!cfg) return alertUser('Keine Benachrichtigungseinstellungen verfügbar.', 'error');
    saveUserSetting(EL_NOTIFY_TEMPLATE_KEY, cfg);
    state.notificationTemplate = JSON.parse(JSON.stringify(cfg));
    alertUser('Vorlage gespeichert.', 'success');
}

function syncArticleEditorFromDom() {
    const editor = state.articleEditor;
    if (!editor) return null;
    const titleInput = document.getElementById('ela-title');
    if (!titleInput) return editor;
    editor.title = String(titleInput.value || '');
    editor.categoryId = String(document.getElementById('ela-cat')?.value || '');
    ...
    if (String(editor.articleMode || 'single') === 'variant') {
        ...
        editor.defaultUnit = 'Stück';
        editor.variants = Array.from(document.querySelectorAll('[data-variant-row]')).map((row) => createEmptyVariant({
            id: row.dataset.variantId,
            brand: row.querySelector('[data-variant-field="brand"]')?.value || '',
            label: row.querySelector('[data-variant-field="label"]')?.value || '',
...
export async function initializeEinkaufsliste() {
    ...
    if (!inited) {
        ...
        const storedSection = getUserSetting(EL_SECTION_KEY, MANAGE[0].id);
        const storedNotifySection = getUserSetting(EL_NOTIFY_SECTION_KEY, ADVANCED[0].id);
        state.section = MANAGE.some((entry) => entry.id === storedSection) ? storedSection : MANAGE[0].id;
        state.notifySection = ADVANCED.some((entry) => entry.id === storedNotifySection) ? storedNotifySection : ADVANCED[0].id;
...
const EL_NOTIFY_SECTION_KEY = 'el_notify_section';
...
function listenActiveList() {
    ...
    activeUnsubs.push(onSnapshot(query(sub(list.id, 'items'), orderBy('createdAt', 'desc')), (s) => { state.items = s.docs.map((d) => ({ ...d.data(), id: d.id, storeIds: d.data().storeIds || [], eanCodes: d.data().eanCodes || [], variantId: String(d.data().variantId || ''), variantBrand: String(d.data().variantBrand || ''), variantLabel: String(d.data().variantLabel || ''), variantStoreId: String(d.data().variantStoreId || '') })); render(); }, (error) => reportListenerError('listenActiveList:items', error)));
    activeUnsubs.push(onSnapshot(query(sub(list.id, 'presence'), orderBy('lastSeen', 'desc')), (s) => { state.presence = s.docs.map((d) => ({ id: d.id, ...d.data() })).filter((x) => Date.now() - (toDate(x.lastSeen)?.getTime() || 0) <= PRESENCE_MS); updatePresenceInlineUi(); }, (error) => reportListenerError('listenActiveList:presence', error)));
...
function renderScannerPanelActive(embedded = false) {
    ...
    return `<div class="${shellClass}"><div class="flex flex-wrap justify-between gap-2 items-center"><div><div class="${titleClass}">${scannerTitle}</div><div class="text-sm text-gray-500">${scannerText}</div></div><button class="elb bg-gray-100 text-gray-700" data-a="close-scan">Schließen</button></div><div class="${videoWrapClass}"><video id="el-video" autoplay playsinline muted></video></div><div class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]"><input id="el-scan-manual" class="eli" placeholder="${isArticleMode ? 'EAN/QR eingeben oder per Scanner senden' : 'EAN/QR manuell eingeben'}"><button class="elb a w-full sm:w-auto" data-a="manual-scan">${manualLabel}</button></div><div id="el-scan-status" class="text-sm text-gray-500">${escapeHtml(state.scanStatus || 'Kamera wird gestartet...')}</div>${isArticleMode ? `<div class="elc space-y-2"><div class="text-xs font-bold uppercase text-gray-500">Erfasste Codes</div><div id="el-scan-collected" class="elm">${state.scanCodes.length ? state.scanCodes.map((code) => chip(`${escapeHtml(code)} <button type="button" class="elchipbtn" data-a="remove-scanned-code" data-id="${escapeHtml(code)}">×</button>`, 'bg-indigo-100 text-indigo-700')).join(' ') : '<span class="text-sm text-gray-400">Noch keine Codes erfasst.</span>'}</div></div><div class="flex flex-col sm:flex-row justify-end gap-2"><button class="elb bg-emerald-600 text-white w-full sm:w-auto" data-a="save-scanned-codes" ${state.scanCodes.length ? '' : 'disabled'}>OK</button><button class="elb bg-indigo-600 text-white w-full sm:w-auto" data-a="save-scanned-codes-next" ${state.scanCodes.length && nextTarget ? '' : 'disabled'}>${nextTarget?.variant ? 'Nächste Variante ohne EAN' : 'Nächsten Artikel ohne EAN'}</button></div>` : ''}</div>`;
}

function renderActionBar() {
    if (state.mode === 'manage' || state.mode === 'notify') return '';
    if (state.listMode === 'search') {
        return `<div class="elc !p-3 space-y-3"><div class="flex justify-center">${renderModeToggle()}</div><div class="eltitlewrap"><input id="el-search" class="eli eltitleinput" placeholder="Suchen oder scannen..." value="${escapeHtml(state.search)}"><button class="eltitlecam ${state.scanOpen ? 'a' : ''}" data-a="open-scan" title="Scanner ${state.scanOpen ? 'deaktivieren' : 'aktivieren'}">📷</button></div>${renderArticleSuggestionList('search')}${renderPresenceInline()}${state.scanOpen && !String(state.scanMode || '').startsWith('article') ? renderScannerPanelActive(true) : ''}</div>`;
    }
    ...
    const showScanButton = !String(state.title || '').trim();
    const selectionBadge = selectionVariant
        ...
        : selectionArticle
            ...
            : '';
    const titlePadLeft = showScanButton ? '2.8rem' : '.75rem';
    const titlePadRight = locked ? '5.75rem' : '.75rem';
    return `<div class="elc !p-3 space-y-3"><div class="flex justify-center">${renderModeToggle()}</div><div class="elinputstack"><div class="elinputgrid"><select id="el-unit" class="els elinput-unit">${UNITS.map((u) => `<option value="${u}" ${state.unit === u ? 'selected' : ''}>${u}</option>`).join('')}</select><input id="el-q" class="eli elinput-qty" value="${escapeHtml(state.q)}" placeholder="Menge"><div class="eltitlewrap"><input id="el-title" class="eli eltitleinput ${locked ? 'bg-slate-100 cursor-not-allowed' : ''}" style="padding-left:${titlePadLeft};padding-right:${titlePadRight};" ${locked ? 'readonly aria-readonly="true"' : ''} placeholder="Artikel eingeben..." value="${escapeHtml(state.title)}"><button id="el-title-scan" class="eltitlecam ${state.scanOpen ? 'a ' : ''}${showScanButton ? '' : 'h'}" data-a="open-scan" title="Scanner ${state.scanOpen ? 'deaktivieren' : 'aktivieren'}">📷</button>${locked ? `<button id="el-title-clear" type="button" class="absolute right-[.45rem] top-1/2 -translate-y-1/2 rounded-full border border-slate-300 bg-slate-100 px-2 py-1 text-[11px] font-black text-slate-600" data-a="clear-input-selection">entfernen</button>` : ''}</div><button class="elb a elinput-add" data-a="add-item" ${!canAdd() ? 'disabled' : ''}>+</button></div><div id="el-title-suggestions">${renderArticleSuggestionList('title')}</div>${selectionBadge ? `<div class="flex flex-wrap gap-2">${selectionBadge}</div>` : ''}<div class="eldetailhead">${renderPresenceInline()}<button class="eldetailtoggle ${state.inputDetailsOpen ? 'o' : ''}" data-a="toggle-input-details">+ Details</button></div>${state.inputDetailsOpen ? `<div class="eldetailpanel"><div class="eldetailrow"><select id="el-store-add" class="els"><option value="">Ohne Geschäft</option>${state.stores.map((s) => `<option value="${s.id}" ${state.storeIds[0] === s.id ? 'selected' : ''}>${escapeHtml(s.name)}</option>`).join('')}</select><input id="el-note" class="eli" placeholder="Anmerkung optional" value="${escapeHtml(state.note)}"></div>${state.storeIds.length > 1 ? `<div class="elm">${state.storeIds.map((id) => chip(`${escapeHtml(state.stores.find((s) => s.id === id)?.name || id)} <button data-a="del-store" data-id="${id}">×</button>`, 'bg-orange-100 text-orange-700')).join(' ')}</div>` : ''}</div>` : ''}${state.scanOpen && !String(state.scanMode || '').startsWith('article') ? renderScannerPanelActive(true) : ''}</div></div>`;
}
...
function categoryChipClass(categoryId = '') {
    ...
    if (!value) return 'bg-slate-100 text-slate-700 ring-1 ring-slate-200';
    const index = Array.from(value).reduce((sum, char) => sum + char.charCodeAt(0), 0) % CATEGORY_CHIP_PALETTE.length;
...
function listSuggestionArticles() {
    ...
    return state.articles.filter((article) => {
        if (articleIds.has(String(article.id || '').trim())) return true;
        const articleTexts = [article.title, ...(article.aliases || [])].map((entry) => String(entry || '').trim().toLowerCase()).filter(Boolean);
        if (articleTexts.some((text) => titles.has(text))) return true;
        return (article.variants || []).some((variant) => buildVariantSearchTerms(article, variant).some((text) => titles.has(String(text || '').trim().toLowerCase())));
    });
}
...
function updateInputTitleUi() {
    ...
    if (titleInput) {
        ...
        titleInput.style.paddingLeft = showScanButton ? '2.8rem' : '.75rem';
        titleInput.style.paddingRight = locked ? '5.75rem' : '.75rem';
    }
    ...
    if (scanButton) scanButton.classList.toggle('h', !showScanButton);
}

function renderPresenceInlineContent() {
    return state.presence.length
        ? state.presence.map((p) => chip(`${escapeHtml(p.userName || p.userId)} (${escapeHtml(p.currentArea || 'Liste')})`, 'bg-emerald-100 text-emerald-700')).join(' ')
        : '<span class="text-xs text-gray-400">Niemand sonst gerade aktiv.</span>';
}

function renderPresenceInline() {
    if (!activeList()) return '';
    return `<div class="elpresence">${renderPresenceInlineContent()}</div>`;
}

function updatePresenceInlineUi() {
    if (!root) return;
    root.querySelectorAll('.elpresence').forEach((host) => {
        host.innerHTML = renderPresenceInlineContent();
    });
}
...
function nextMissingEanTarget(currentArticleId = '', currentVariantId = '') {
    const targets = [];
    ...
    const index = targets.findIndex((target) => target.articleId === currentArticleId && target.variantId === currentVariantId);
...
async function refreshVisibleSyncPreviewForTargets(targetListIds = []) {
    const currentTargetId = syncTargetListId();
    const normalizedTargets = normalizedAutoSyncTargetIds(targetListIds);
    if (!currentTargetId || !normalizedTargets.includes(currentTargetId) || !state.syncCenterOpen) return;
    await refreshSyncPreview(currentTargetId);
}

function showSyncEntryInfo(type, id) {
    const preview = syncPreviewEntry(type, id);
    if (!preview.info) return;
    if (typeof window !== 'undefined' && typeof window.alert === 'function') {
        window.alert(preview.info);
        return;
    }
    alertUser(preview.info, 'info');
}

async function syncTypeEntriesToAutoTargets(type, entries = [], targetListIds = state.autoSyncTargets, { refreshPreview = true } = {}) {
    const list = activeList();
    if (!list?.id || list.ownerId !== uid()) return 0;
    const normalizedTargets = normalizedAutoSyncTargetIds(targetListIds);
    const normalizedEntries = Array.from(new Map((entries || []).filter(Boolean).map((entry) => [String(entry?.id || entry?.masterId || '').trim(), entry])).values()).filter((entry) => String(entry?.id || entry?.masterId || '').trim());
    if (!normalizedTargets.length || !normalizedEntries.length) return 0;
    let moved = 0;
    for (const targetListId of normalizedTargets) {
    ...
    if (refreshPreview) await refreshVisibleSyncPreviewForTargets(normalizedTargets);
    return moved;
}

async function syncAllMastersToAutoTargets(targetListIds = state.autoSyncTargets, { silent = false } = {}) {
    const normalizedTargets = normalizedAutoSyncTargetIds(targetListIds);
    if (!normalizedTargets.length) return 0;
    const types = ['categories', 'stores', 'remarks', 'notes', 'articles'];
    let moved = 0;
    for (const type of types) {
        moved += await syncTypeEntriesToAutoTargets(type, syncEntriesByType(type), normalizedTargets, { refreshPreview: false });
    }
    await refreshVisibleSyncPreviewForTargets(normalizedTargets);
    if (!silent) alertUser(moved ? `${moved} Stammdatensätze per Auto-Sync abgeglichen.` : 'Keine Stammdaten für den Auto-Sync gefunden.', moved ? 'success' : 'info');
    return moved;
}
...
async function toggleAutoSyncLink(targetListId) {
    const list = activeList();
    const normalizedTargetListId = String(targetListId || '').trim();
    const targetList = state.lists.find((entry) => entry.id === normalizedTargetListId && entry.ownerId === uid());
    if (!list || list.ownerId !== uid() || !targetList) return;
    const enabled = !state.autoSyncTargets.includes(normalizedTargetListId);
    const payload = {
    ...
    if (enabled) {
        state.autoSyncTargets = Array.from(new Set([...state.autoSyncTargets, normalizedTargetListId]));
        render();
        await Promise.all([
            setDoc(doc(syncTargetsRef(list.id), normalizedTargetListId), payload, { merge: true }),
            deleteDoc(doc(syncTargetsRef(normalizedTargetListId), list.id))
        ]);
        state.autoSyncOpen = true;
        await syncAllMastersToAutoTargets([normalizedTargetListId], { silent: true });
        alertUser(`Auto-Sync zu ${targetList.name || 'Zielliste'} aktiviert. Bestehende Stammdaten wurden sofort abgeglichen.`, 'success');
        return;
    }
    state.autoSyncTargets = state.autoSyncTargets.filter((entry) => entry !== normalizedTargetListId);
    render();
    await Promise.all([
        deleteDoc(doc(syncTargetsRef(list.id), normalizedTargetListId)),
        deleteDoc(doc(syncTargetsRef(normalizedTargetListId), list.id))
    ]);
...
function renderSyncCenterSection(list = activeList()) {
    ...
    const renderTypeCard = (type) => {
        ...
        const content = entries.length
            ? entries.map((entry) => {
                ...
                const selectable = syncTransferableMode(preview.mode);
                const infoTone = preview.mode === 'changed' ? 'bg-amber-200 text-amber-900 border border-amber-300' : 'bg-slate-200 text-slate-600 border border-slate-300';
                const buttonClass = !selectable
                ...
                const infoButton = preview.mode === 'changed' || preview.mode === 'same'
                    ? `<button type="button" class="inline-flex h-7 w-7 items-center justify-center rounded-full text-[11px] font-black ${infoTone}" data-a="show-sync-entry-info" data-type="${type}" data-id="${entry.id}" title="${escapeHtml(preview.info || '')}">i</button>`
                    : '';
    ...
    return `<div class="space-y-4">${stepOne}${stepTwo}${stepThree}${autoPanel}</div>`;
}
...
function renderManageSections() {
    return `<div class="elc"><div class="elm justify-center">${MANAGE.map((s) => `<button class="elb ${state.section === s.id ? 'a' : 'bg-gray-100 text-gray-700'}" data-a="manage-section" data-v="${s.id}">${s.label}</button>`).join('')}</div></div>`;
}

function renderAdvancedSections() {
    return `<div class="elc"><div class="elm justify-center">${ADVANCED.map((s) => `<button class="elb ${state.notifySection === s.id ? 'a' : 'bg-gray-100 text-gray-700'}" data-a="notify-section" data-v="${s.id}">${s.label}</button>`).join('')}</div></div>`;
}
...
function render() {
    ...
    main.innerHTML = `
        ...
        ${state.mode === 'notify' ? renderAdvancedSections() : ''}
...
function renderBody() {
    const list = activeList();
    if (!list) return '<div class="elc text-sm text-gray-500">Bitte zuerst eine Liste auswählen.</div>';
    if (state.mode === 'notify') return renderAdvancedBody();
    if (state.mode === 'shop') {
...
function renderManageContent() {
    ...
    if (state.section === 'stores') return renderManageStoresSection();
    if (state.section === 'categories') return renderManageCategoriesSection();
    if (state.section === 'remarks') return renderManageRemarksSection();
    if (state.section === 'notes') return renderManageNotesSection();
    return renderManageArticlesSection();
}

function renderAdvancedBody() {
    if (state.notifySection === 'notification') return renderNotificationCenter();
    if (!canManage()) return '<div class="elc text-sm text-red-700">Keine Verwaltungsberechtigung für diese Liste.</div>';
    if (state.notifySection === 'sync') return renderAdvancedSyncSection();
    if (state.notifySection === 'settings') return renderAdvancedSettingsSection();
    return renderManageGeneralSection();
}
...
function renderManageGeneralSection() {
    const list = activeList();
    if (!list) return '<div class="elc text-sm text-gray-500">Bitte zuerst eine Liste auswählen.</div>';
...
function renderAdvancedSettingsSection() {
    ...
    return `<div class="space-y-3"><div><div class="font-black text-sm text-gray-900">Einkaufslisten verwalten</div><div class="text-xs text-gray-500">Privatliste ist fix, nicht umbenennbar und nicht teilbar.</div></div><div class="grid gap-4 lg:grid-cols-[320px_minmax(0,1fr)]"><div class="elc space-y-3"><div class="flex flex-wrap justify-between gap-2 items-center"><div class="font-black text-sm">Listen</div><button class="elb a" data-a="create-list" ${!(currentUser.permissions || []).includes('EINKAUFSLISTE_CREATE') && currentUser.role !== 'SYSTEMADMIN' ? 'disabled' : ''}>+ Liste</button></div>${state.lists.map((entry) => `<button class="w-full text-left rounded-2xl border p-3 ${state.listId === entry.id ? 'ring-2 ring-indigo-500 border-indigo-300' : 'border-gray-200'}" data-a="list" data-id="${entry.id}"><div class="font-bold text-sm">${escapeHtml(entry.name)}</div><div class="text-xs text-gray-500">${entry.isPrivateSystemList ? 'Privatliste' : 'Normale Liste'} · ${entry.active !== false ? 'aktiv' : 'pausiert'}</div></button>`).join('')}</div><div class="space-y-4">${list ? `<div class="elc space-y-3"><div class="font-black text-sm">Allgemein</div><div class="grid gap-3 sm:grid-cols-2"><input id="el-set-name" class="eli" value="${escapeHtml(list.name || '')}" ${list.isPrivateSystemList || list.ownerId !== uid() ? 'disabled' : ''}><label class="inline-flex items-center gap-2 text-sm font-bold text-gray-700"><input type="checkbox" id="el-set-active" ${list.active !== false ? 'checked' : ''} ${list.isPrivateSystemList || list.ownerId !== uid() ? 'disabled' : ''}> aktiv</label></div>${list.ownerId === uid() && !list.isPrivateSystemList ? `<div class="flex flex-wrap gap-2"><button class="elb a" data-a="save-list">Liste speichern</button><button class="elb bg-red-600 text-white" data-a="delete-list" data-id="${list.id}">Liste löschen</button></div>` : '<div class="text-sm text-gray-400">Nur der Listeninhaber kann diese Liste ändern.</div>'}</div><div class="elc space-y-3"><div class="font-black text-sm">Freigaben</div><div class="grid gap-2 sm:grid-cols-2"><select id="el-c-user" class="els"><option value="">Person wählen...</option>${candidates.map((entry) => `<option value="${entry.id}" ${state.collab.userId === entry.id ? 'selected' : ''}>${escapeHtml(entry.name)}</option>`).join('')}</select><input id="el-c-from" type="datetime-local" class="eli" value="${escapeHtml(state.collab.accessFrom || dtLocal(new Date()))}"><input id="el-c-until" type="datetime-local" class="eli" value="${escapeHtml(state.collab.accessUntil || '')}"><div class="elm"><label class="inline-flex items-center gap-1 text-xs font-bold text-gray-600"><input id="el-cr" type="checkbox" ${state.collab.canRead ? 'checked' : ''}> Lesen</label><label class="inline-flex items-center gap-1 text-xs font-bold text-gray-600"><input id="el-ca" type="checkbox" ${state.collab.canAdd ? 'checked' : ''}> Hinzufügen</label><label class="inline-flex items-center gap-1 text-xs font-bold text-gray-600"><input id="el-cs" type="checkbox" ${state.collab.canShop ? 'checked' : ''}> Einkaufen</label><label class="inline-flex items-center gap-1 text-xs font-bold text-gray-600"><input id="el-cm" type="checkbox" ${state.collab.canManage ? 'checked' : ''}> Verwalten</label><label class="inline-flex items-center gap-1 text-xs font-bold text-gray-600"><input id="el-cw" type="checkbox" ${state.collab.canManageWrite ? 'checked' : ''}> Schreibrechte</label></div></div><button class="elb a" data-a="save-collab" ${list.ownerId !== uid() || list.isPrivateSystemList ? 'disabled' : ''}>Freigabe speichern</button>${permissionEntries.length ? permissionEntries.map((entry) => `<div class="rounded-2xl border border-gray-200 p-3"><div class="flex flex-wrap justify-between gap-2 items-center"><div><div class="font-bold text-sm">${escapeHtml(entry.userName || entry.userId)}</div><div class="text-xs text-gray-500">${dt(entry.accessFrom)} bis ${entry.accessUntil ? dt(entry.accessUntil) : 'offen'}</div></div><button class="elb bg-red-600 text-white" data-a="del-collab" data-id="${entry.userId}" ${list.ownerId !== uid() || list.isPrivateSystemList ? 'disabled' : ''}>Entfernen</button></div></div>`).join('') : '<div class="text-sm text-gray-400">Keine Freigaben.</div>'}</div>` : '<div class="elc text-sm text-gray-400">Bitte zuerst eine Liste wählen.</div>'}</div></div></div>`;
}

function renderManageCategoriesSection() {
    return `<div class="space-y-3"><div class="elc flex flex-wrap gap-2"><input id="el-draft-category" class="eli" placeholder="Neue Kategorie" value="${escapeHtml(state.drafts.category)}"><button class="elb a" data-a="add-category" ${!canManageWrite() ? 'disabled' : ''}>+ Kategorie</button></div>${state.categories.map((c) => `<div class="elc flex flex-wrap justify-between gap-2"><div class="font-bold text-sm">${escapeHtml(c.name)}</div><button class="elb bg-red-600 text-white" data-a="del-category" data-id="${c.id}" ${!canManageWrite() ? 'disabled' : ''}>Löschen</button></div>`).join('') || '<div class="elc text-sm text-gray-400">Keine Kategorien vorhanden.</div>'}</div>`;
}

function renderManageRemarksSection() {
    return `<div class="space-y-3"><div class="elc flex flex-wrap gap-2"><input id="el-draft-remark" class="eli" placeholder="Häufige Anmerkung" value="${escapeHtml(state.drafts.remark)}"><button class="elb a" data-a="add-remark" ${!canManageWrite() ? 'disabled' : ''}>+ Anmerkung</button></div>${state.remarks.map((n) => `<div class="elc flex flex-wrap justify-between gap-2"><div class="font-bold text-sm">${escapeHtml(n.text || n.name || '')}</div><button class="elb bg-red-600 text-white" data-a="del-remark" data-id="${n.id}" ${!canManageWrite() ? 'disabled' : ''}>Löschen</button></div>`).join('') || '<div class="elc text-sm text-gray-400">Keine Anmerkungen.</div>'}</div>`;
}

function renderManageNotesSection() {
...
function renderItem(item) {
    ...
    const categoryId = String(item.categoryId || article?.categoryId || '').trim();
    ...
    return `<div class="elitem border-t border-gray-100 flex items-start gap-2"><button type="button" class="text-left min-w-0 flex-1" data-a="edit-item" data-id="${item.id}" title="Doppelklick für Bearbeitungsaktionen"><div class="flex flex-wrap items-center gap-2"><div class="min-w-0 flex-1 flex items-center gap-2"><div class="min-w-0 truncate font-bold text-sm text-gray-900">${escapeHtml(ell(item.title, 40))}</div>${metaIcons}</div>${locked ? chip(`gesperrt von ${escapeHtml(lock.userName || lock.userId)}`, 'bg-red-100 text-red-700') : ''}${restItem ? chip('Rest', 'bg-orange-100 text-orange-800 ring-1 ring-orange-300') : ''}</div>${restoredInfo ? `<div class="mt-1 flex flex-wrap items-center gap-2"><span class="text-xs text-gray-500">${restoredInfo}</span></div>` : ''}${item.persistentNote ? `<div class="text-xs text-gray-600 mt-2">${escapeHtml(item.persistentNote)}</div>` : ''}${item.note ? `<div class="text-xs text-gray-600 mt-1">${escapeHtml(item.note)}</div>` : ''}</button><div class="flex items-center justify-end gap-1 sm:gap-2 flex-wrap sm:flex-nowrap shrink-0">${category ? chip(escapeHtml(category.name), `${categoryChipClass(category.id)} whitespace-nowrap`) : ''}${chip(escapeHtml(quantityLabel), 'bg-indigo-50 text-indigo-700 whitespace-nowrap')}${qtyActionVisible ? `<button class="elaction elaction-qty text-[11px] sm:text-xs px-2 sm:px-3 whitespace-nowrap" data-a="quantity" data-id="${item.id}" title="Menge übernehmen"><span class="sm:hidden">Menge</span><span class="hidden sm:inline">Menge übernehmen</span></button>` : ''}${controls}</div></div>`;
}
...
async function addMaster(name, value, key) {
    ...
    await syncTypeEntriesToAutoTargets(name, [normalizeMasterEntry(name, { [key]: v, text: v, name: v, masterId: ref.id, listId: state.listId }, ref.id)]);
    return ref;
}
...
async function addFree(ref, value) {
    ...
    await syncTypeEntriesToAutoTargets(name, [normalizeMasterEntry(name, { text: v, masterId: targetRef.id, listId: state.listId }, targetRef.id)]);
}
...
async function toggleStoreCategory(storeId, catId, checked) {
    const store = state.stores.find((s) => s.id === storeId); if (!store) return; const next = checked ? Array.from(new Set([...(store.categoryOrder || []), catId])) : (store.categoryOrder || []).filter((x) => x !== catId); const updatedStore = normalizeMasterEntry('stores', { ...store, categoryOrder: next }, storeId); await updateDoc(doc(master('stores'), storeId), buildMasterWritePayload('stores', { ...store, categoryOrder: next }, { masterId: store.masterId || store.id, keepCreatedBy: true })); await syncTypeEntriesToAutoTargets('stores', [updatedStore]); await logActivity('Geschäftskategorie geändert', { storeId, catId, checked });
}
...
async function saveStoreCategoryEditorActive() {
    ...
    await syncTypeEntriesToAutoTargets('stores', [normalizeMasterEntry('stores', { ...store, categoryOrder: editor.categoryOrder }, editor.storeId)]);
...
async function onClickLegacy(e) {
    ...
    if (a === 'manage-section') { state.section = MANAGE.some((entry) => entry.id === btn.dataset.v) ? btn.dataset.v : MANAGE[0].id; saveUserSetting(EL_SECTION_KEY, state.section); touchPresence(); render(); return; }
    if (a === 'notify-section') { state.notifySection = ADVANCED.some((entry) => entry.id === btn.dataset.v) ? btn.dataset.v : ADVANCED[0].id; saveUserSetting(EL_NOTIFY_SECTION_KEY, state.notifySection); touchPresence(); render(); return; }
    ...
    if (a === 'open-settings') { state.mode = 'notify'; state.notifySection = 'settings'; state.settingsOpen = false; saveUserSetting(EL_MODE_KEY, state.mode); saveUserSetting(EL_NOTIFY_SECTION_KEY, state.notifySection); touchPresence(); render(); return; }
...
function resolveInputSelection() {
    let article = state.selectedArticleId ? state.articles.find((entry) => entry.id === state.selectedArticleId) : null;
    let variant = article ? findVariantById(article, state.selectedVariantId) : null;
    if (!article) {
        const exact = findExactArticleSelection(state.title);
...
function renderUnknown() {
    const el = document.getElementById('el-unknown');
    if (!el) return;
    const article = state.articles.find((entry) => entry.id === state.unknownArticleId);
    const showVariants = !!article && isVariantArticle(article);
    el.className = `elmodal ${state.unknownCode ? 'o' : ''}`;
    if (!state.unknownCode) { el.innerHTML = ''; return; }
    el.innerHTML = `<div class="elpanel p-4 sm:p-5 space-y-4"><div class="flex flex-wrap justify-between gap-2 items-center"><div><div class="text-xl font-black text-gray-900">Unbekannter Code</div><div class="text-sm text-gray-500">Bitte zuerst einem bestehenden Artikel oder einer Variante zuordnen.</div></div><button class="elb bg-gray-100 text-gray-700" data-a="close-unknown">Abbruch</button></div><div class="elc bg-slate-50 text-sm space-y-3"><div><b>Code:</b> ${escapeHtml(state.unknownCode)}</div><select id="el-unknown-article" class="els"><option value="">Bestehendem Artikel zuordnen...</option>${state.articles.map((entry) => `<option value="${entry.id}" ${state.unknownArticleId === entry.id ? 'selected' : ''}>${escapeHtml(entry.title)}</option>`).join('')}</select>${showVariants ? `<select id="el-unknown-variant" class="els"><option value="">Variante auswählen...</option>${(article.variants || []).map((variant) => `<option value="${variant.id}" ${state.unknownVariantId === variant.id ? 'selected' : ''}>${escapeHtml(buildVariantGeneratedTitle(article, variant))} · ${escapeHtml(state.stores.find((store) => store.id === variant.storeId)?.name || 'Ohne Geschäft')}</option>`).join('')}</select>` : ''}<div class="flex flex-wrap justify-end gap-2"><button class="elb bg-gray-100 text-gray-700" data-a="close-unknown">Abbruch</button><button class="elb bg-emerald-600 text-white" data-a="save-unknown">Zuordnen</button></div></div></div>`;
}
...
async function addItem() {
    ...
    const title = String(state.title || '').trim();
    if (!title) return alertUser('Bitte Artikel eingeben.', 'error');
    let { article, variant } = resolveInputSelection();
    if (article && isVariantArticle(article) && !variant) {
        state.variantPickerArticleId = article.id;
        state.inputDetailsOpen = true;
        render();
    ...
    if (!article) {
        ...
        await syncTypeEntriesToAutoTargets('articles', [article]);
        variant = null;
    }
    ...
    const unit = String(state.unit || variant?.unit || article.defaultUnit || 'Stück') || 'Stück';
    const storeIds = state.storeIds.length ? [...state.storeIds] : (variant?.storeId ? [variant.storeId] : [...(article.storeIds || [])]);
    const persistentNote = articlePrefillNote(article);
    const note = String(state.note || '').trim();
    const itemNote = note && note === persistentNote ? '' : note;
...
function openPurchase(itemOrArticle, isScan) {
    ...
    const article = item ? state.articles.find((a) => a.id === item.articleId) : itemOrArticle;
    const variant = item
        ? (item.variantId ? (findVariantById(article, item.variantId) || { id: item.variantId, brand: item.variantBrand || '', label: item.variantLabel || '', storeId: item.variantStoreId || '', quantity: item.quantity, unit: item.unit, eanCodes: item.eanCodes || [] }) : null)
        : (itemOrArticle?._variant || null);
    const title = item?.title || (variant ? buildVariantGeneratedTitle(article, variant) : article?.title || 'Artikel');
    const unit = item?.unit || variant?.unit || article?.defaultUnit || 'Stück';
    const base = Number(item?.quantity || variant?.quantity || article?.defaultQuantity || 1);
    const startQuantity = item ? Number(item.quantity || 0) : base;
    state.purchase = { kind: isScan ? 'scan' : 'manual', itemId: item?.id || '', articleId: item?.articleId || article?.id || '', variantId: String(variant?.id || ''), variantBrand: String(variant?.brand || ''), variantLabel: String(variant?.label || ''), variantStoreId: String(variant?.storeId || ''), title, unit, target: item ? Number(item.quantity || 0) : 0, base, raw: '', quantity: startQuantity, pristine: true, eanCodes: [...(variant?.eanCodes || article?.eanCodes || [])], storeIds: variant?.storeId ? [variant.storeId] : [...(article?.storeIds || [])] };
    renderPurchase();
    resetAutoScan(isScan);
}
...
async function confirmPurchase() {
    ...
    if (!(qty > 0)) return alertUser('Bitte eine Menge eingeben.', 'error');
    closePurchaseModal();
    if (p.itemId) {
        const item = state.items.find((x) => x.id === p.itemId);
        if (!item) return;
        hideItemActionButtons(item.id);
        hideQuantityActionButton(item.id);
        if (qty >= Number(item.quantity || 0)) {
        ...
        if (!article) return;
        const open = state.items.find((x) => x.articleId === article.id && x.status !== 'checked' && String(x.variantId || '') === String(p.variantId || ''));
        if (open) {
            openPurchase(open, true);
            state.purchase.quantity = qty;
            await confirmPurchase();
...
async function saveScannedCodesActive(openNext = false) {
    ...
    flashScanSuccess();
    if (openNext) {
        const nextTarget = nextMissingEanTarget(article.id, state.scanVariantId);
        if (nextTarget) {
            state.scanArticleId = nextTarget.articleId;
            state.scanVariantId = nextTarget.variantId;
            state.scanCodes = [];
            state.scanStatus = `Nächstes Ziel: ${nextTarget.label}`;
            state.scanLastCode = '';
            state.scanLastAt = 0;
            render();
            return;
        ...
        closeScannerModal();
        alertUser('Keine weiteren Artikel oder Varianten ohne EAN gefunden.', 'success');
        return;
    }
    closeScannerModal();
}
...
async function handleScanCode(code) {
    const { article, variant } = findArticleByCode(code);
    if (!article) {
        state.scanStatus = `EAN nicht gefunden: ${code}`;
        updateScannerDynamicUi();
        alertUser('Kein Artikel zu diesem EAN-Code gefunden.', 'error');
    ...
    state.scanStatus = `Gefunden: ${variant ? buildVariantGeneratedTitle(article, variant) : article.title}`;
    updateScannerDynamicUi();
    flashScanSuccess();
    if (variant) {
        openPurchase({ ...article, title: buildVariantGeneratedTitle(article, variant), defaultQuantity: Number(variant.quantity || 1), defaultUnit: variant.unit || 'Stück', storeIds: variant.storeId ? [variant.storeId] : [], eanCodes: [...(variant.eanCodes || [])], _variant: variant }, true);
        return;
    }
    openPurchase(article, true);
}
...
async function saveArticle() {
    const draft = normalizeArticle(syncArticleEditorFromDom() || state.articleEditor || createEmptyArticleEditor());
    ...
    if (draft.id) {
        await updateDoc(doc(master('articles'), draft.id), buildMasterWritePayload('articles', { ...draft, ...payload }, { masterId: draft.masterId || draft.id, keepCreatedBy: true }));
        savedArticle = normalizeArticle({ ...draft, ...payload, id: draft.id, masterId: draft.masterId || draft.id, listId: state.listId });
    } else {
        const ref = doc(master('articles'));
        await setDoc(ref, { ...buildMasterWritePayload('articles', payload, { masterId: ref.id }), createdAt: serverTimestamp() });
        savedArticle = normalizeArticle({ ...payload, id: ref.id, masterId: ref.id, listId: state.listId });
    }
    await syncTypeEntriesToAutoTargets('articles', savedArticle ? [savedArticle] : []);
...
function onInput(e) {
    ...
    if (t.id === 'el-title') {
        state.title = t.value;
        const article = state.selectedArticleId ? state.articles.find((entry) => entry.id === state.selectedArticleId) : null;
        const variant = article ? findVariantById(article, state.selectedVariantId) : null;
        const expected = variant ? buildVariantGeneratedTitle(article, variant) : String(article?.title || '');
        if (expected && String(t.value || '').trim() !== expected.trim()) resetInputSelection();
        updateInputTitleUi();
        return;
    }
    if (t.id === 'el-note') state.note = t.value;
...
function onChange(e) {
    ...
    if (t.id === 'el-store-numbers') { state.storeNumbers = t.checked; saveUserSetting(EL_STORE_NUMBERS_KEY, state.storeNumbers); render(); return; }
    if (t.id === 'el-missing-ean') { state.missingEanOnly = t.checked; render(); return; }
    if (t.id === 'el-unknown-article') { state.unknownArticleId = t.value; state.unknownVariantId = ''; renderUnknown(); return; }
    if (t.id === 'el-unknown-variant') { state.unknownVariantId = t.value; return; }
    if (t.id === 'el-c-user') state.collab.userId = t.value;
    if (t.id === 'el-c-from') state.collab.accessFrom = t.value;
    if (t.id === 'el-c-until') state.collab.accessUntil = t.value;
    if (t.id === 'el-cr') state.collab.canRead = t.checked;
...
async function onClick(e) {
    ...
    if (a === 'close-variant-picker') { state.variantPickerArticleId = ''; render(); return; }
    if (a === 'pick-variant-choice') {
        const article = state.articles.find((x) => x.id === btn.dataset.id);
        const variant = article ? findVariantById(article, btn.dataset.variant) : null;
        if (article && variant) {
            prefillInputFromVariant(article, variant);
            requestFocusAfterRender('el-title');
            render();
        }
        return;
    ...
    if (a === 'article-mode') {
        ...
        state.articleEditor.articleMode = btn.dataset.v === 'variant' ? 'variant' : 'single';
        if (state.articleEditor.articleMode === 'variant' && !(state.articleEditor.variants || []).length) state.articleEditor.variants = [createEmptyVariant()];
        renderArticle();
        return;
    }
    if (a === 'add-article-variant') {
        syncArticleEditorFromDom();
        state.articleEditor.variants = [...(state.articleEditor.variants || []), createEmptyVariant()];
        renderArticle();
        return;
    }
    if (a === 'remove-article-variant') {
        syncArticleEditorFromDom();
        state.articleEditor.variants = (state.articleEditor.variants || []).filter((variant) => variant.id !== btn.dataset.variant);
        if (!state.articleEditor.variants.length) state.articleEditor.variants = [createEmptyVariant()];
        renderArticle();
        return;
    }
    if (a === 'capture-variant-ean') {
        syncArticleEditorFromDom();
        openScanner('article-variant-ean', btn.dataset.id, btn.dataset.variant);
        return;
    }
    if (a === 'capture-ean' && btn.dataset.id) { openScanner('article-ean', btn.dataset.id); return; }
    if (a === 'art-store') {
        syncArticleEditorFromDom();
        const ids = state.articleEditor.storeIds || [];
        state.articleEditor.storeIds = ids.includes(btn.dataset.id) ? ids.filter((x) => x !== btn.dataset.id) : [...ids, btn.dataset.id];
        renderArticle();
        return;
    }
    if (a === 'save-article') { await saveArticle(); return; }
