...
const MANAGE = [{ id: 'general', label: 'Allgemein' }, { id: 'stores', label: 'GeschÃ¤ftewartung' }, { id: 'articles', label: 'Artikelwartung' }, { id: 'categories', label: 'Kategorienwartung' }, { id: 'remarks', label: 'Anmerkungswartung' }, { id: 'notes', label: 'Notizwartung' }];
const UNITS = ['StÃ¼ck', 'Kg', 'Gramm', 'Liter', 'Milliliter', 'Bund', 'Netz', 'Sack', 'Dose', 'Flasche', 'Becher', 'Packung'];
const DEFAULTS = {
    categories: ['Obst', 'GemÃ¼se', 'TiefkÃ¼hl', 'SÃ¼ÃŸigkeiten', 'Saison', 'Hygiene', 'Haushalt', 'GetrÃ¤nke'],
    stores: ['Billa', 'BIPA', 'Penny', 'Lidl', 'Hofer'],
    remarks: ['In Aktion kaufen', 'MHD schauen', 'RÃ¼cksprache halten!'],
    notes: ['Gutschein verwenden', 'Angebote prÃ¼fen']
};
const HOLD_MS = 2000;
...
const dt = (v) => { const d = toDate(v); return d && !Number.isNaN(d.getTime()) ? new Intl.DateTimeFormat('de-AT', { dateStyle: 'short', timeStyle: 'short', timeZone: 'Europe/Vienna' }).format(d) : 'â€”'; };
...
const ell = (s, n = 34) => String(s || '').length > n ? `${String(s || '').slice(0, n - 1)}â€¦` : String(s || '');
...
export async function initializeEinkaufsliste() {
    ...
    try {
        ...
        if (!tokenResult?.claims?.appUserId || tokenResult.claims.appUserId !== uid()) {
            root.innerHTML = '<div class="elc text-sm text-amber-700">Anmeldung noch nicht vollstÃ¤ndig. Bitte neu anmelden und die Einkaufsliste erneut Ã¶ffnen.</div>';
...
async function touchPresence() {
    const list = activeList();
    if (!list) return;
    await setDoc(doc(sub(list.id, 'presence'), uid()), { userId: uid(), userName: uname(), currentArea: state.mode === 'manage' ? MANAGE.find((x) => x.id === state.section)?.label || 'Verwaltung' : (state.listMode === 'input' ? 'Listenmodus Â· Eingeben' : 'Listenmodus Â· Suchen'), lastSeen: serverTimestamp() }, { merge: true });
}
...
function groupedOpen() {
    ...
    if (state.storeDisplay === 'combined') return [{ id: 'combined', label: 'Alle GeschÃ¤fte', note: '', items: sortItemsForShopDisplay(items) }];
    ...
    const none = { id: 'none', label: 'Ohne GeschÃ¤ft', note: '', items: [] };
...
function renderScannerPanelActive(embedded = false) {
    ...
    const scannerTitle = isArticleMode ? `EAN zuordnen: ${escapeHtml(article?.title || 'Artikel')}` : state.scanMode === 'list-add' ? 'Scanner Â· Eingeben' : 'Scanner Â· Listenmodus';
    const scannerText = isArticleMode ? 'Mehrere Codes kÃ¶nnen gesammelt und gemeinsam gespeichert werden.' : state.scanMode === 'list-add' ? 'Gefundene EANs werden direkt mit Menge, Einheit, GeschÃ¤ft und Anmerkung hinzugefÃ¼gt.' : 'Gefundene EANs Ã¶ffnen die MengenÃ¼bernahme. Der Scan bleibt aktiv.';
    const manualLabel = isArticleMode ? 'Code hinzufÃ¼gen' : state.scanMode === 'list-add' ? 'Artikel hinzufÃ¼gen' : 'Code Ã¼bernehmen';
    const shellClass = embedded ? 'mt-3 max-w-lg rounded-2xl border border-slate-200 bg-slate-50 p-3 space-y-3' : 'elpanel p-4 sm:p-5 space-y-4';
    const titleClass = embedded ? 'text-base font-black text-gray-900' : 'text-xl font-black text-gray-900';
    const videoWrapClass = embedded ? 'elcam mx-auto max-w-md' : 'elcam';
    return `<div class="${shellClass}"><div class="flex flex-wrap justify-between gap-2 items-center"><div><div class="${titleClass}">${scannerTitle}</div><div class="text-sm text-gray-500">${scannerText}</div></div><button class="elb bg-gray-100 text-gray-700" data-a="close-scan">SchlieÃŸen</button></div><div class="${videoWrapClass}"><video id="el-video" autoplay playsinline muted></video></div><div class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]"><input id="el-scan-manual" class="eli" placeholder="${isArticleMode ? 'EAN/QR eingeben oder per Scanner senden' : 'EAN/QR manuell eingeben'}"><button class="elb a" data-a="manual-scan">${manualLabel}</button></div><div id="el-scan-status" class="text-sm text-gray-500">${escapeHtml(state.scanStatus || 'Kamera wird gestartet...')}</div>${isArticleMode ? `<div class="elc space-y-2"><div class="text-xs font-bold uppercase text-gray-500">Erfasste Codes</div><div id="el-scan-collected" class="elm">${state.scanCodes.length ? state.scanCodes.map((code) => chip(`${escapeHtml(code)} <button type="button" class="elchipbtn" data-a="remove-scanned-code" data-id="${escapeHtml(code)}">Ã—</button>`, 'bg-indigo-100 text-indigo-700')).join(' ') : '<span class="text-sm text-gray-400">Noch keine Codes erfasst.</span>'}</div></div><div class="flex flex-wrap justify-end gap-2"><button class="elb bg-emerald-600 text-white" data-a="save-scanned-codes" ${state.scanCodes.length ? '' : 'disabled'}>OK</button><button class="elb bg-indigo-600 text-white" data-a="save-scanned-codes-next" ${state.scanCodes.length && nextMissingEanArticle(state.scanArticleId) ? '' : 'disabled'}>NÃ¤chsten Artikel ohne EAN</button></div>` : ''}</div>`;
}

function renderActionBar() {
    if (state.mode === 'manage') return '';
    if (state.listMode === 'search') {
        return `<div class="elc !p-3"><div class="grid gap-3 lg:grid-cols-[auto_minmax(0,1fr)] items-center"><div class="flex justify-center lg:justify-start">${renderModeToggle()}</div><div class="mx-auto flex w-full max-w-3xl items-center gap-2"><input id="el-search" class="eli text-center" placeholder="Suchen oder scannen..." value="${escapeHtml(state.search)}"><button class="elb ${cameraButtonClass()}" data-a="open-scan" title="Scanner ${state.scanOpen ? 'deaktivieren' : 'aktivieren'}">ðŸ“·</button></div></div>${state.scanOpen && state.scanMode !== 'article-ean' ? renderScannerPanelActive(true) : ''}</div>`;
    }
    return `<div class="elc !p-3"><div class="grid gap-3 lg:grid-cols-[220px_minmax(0,1fr)] items-start"><div class="space-y-2"><div class="flex justify-center lg:justify-start">${renderModeToggle()}</div><div class="mx-auto grid max-w-[210px] grid-cols-[88px_110px] gap-2 lg:mx-0"><input id="el-q" class="eli text-center" value="${escapeHtml(state.q)}" placeholder="Menge"><select id="el-unit" class="els">${UNITS.map((u) => `<option value="${u}" ${state.unit === u ? 'selected' : ''}>${u}</option>`).join('')}</select></div></div><div class="mx-auto w-full max-w-5xl space-y-2"><div class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto] items-center"><select id="el-store-add" class="els"><option value="">GeschÃ¤ft zuordnen...</option>${state.stores.map((s) => `<option value="${s.id}">${escapeHtml(s.name)}</option>`).join('')}</select><input id="el-note" class="eli" placeholder="Anmerkung optional" value="${escapeHtml(state.note)}"><button class="elb ${cameraButtonClass()}" data-a="open-scan" title="Scanner ${state.scanOpen ? 'deaktivieren' : 'aktivieren'}">ðŸ“·</button></div><div class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_56px] items-center"><input id="el-title" class="eli" placeholder="Artikel eingeben..." value="${escapeHtml(state.title)}"><button class="elb a !px-0" data-a="add-item" ${!canAdd() ? 'disabled' : ''}>+</button></div>${state.storeIds.length ? `<div class="elm">${state.storeIds.map((id) => chip(`${escapeHtml(state.stores.find((s) => s.id === id)?.name || id)} <button data-a="del-store" data-id="${id}">Ã—</button>`, 'bg-orange-100 text-orange-700')).join(' ')}</div>` : '<div class="text-xs text-gray-400">Optional einem oder mehreren GeschÃ¤ften zuordnen.</div>'}${state.scanOpen && state.scanMode !== 'article-ean' ? renderScannerPanelActive(true) : ''}</div></div>`;
}
...
function openScanner(mode = 'shopping', articleId = '') {
    ...
    state.scanCodes = [];
    state.scanStatus = mode === 'article-ean' ? 'Kamera wird fÃ¼r die EAN-Erfassung gestartet...' : mode === 'list-add' ? 'Kamera wird gestartet. Gefundene EANs werden direkt hinzugefÃ¼gt...' : 'Kamera wird gestartet. Gefundene EANs Ã¶ffnen die MengenÃ¼bernahme...';
    state.scanLastCode = '';
    state.scanLastAt = 0;
    render();
}
...
function updateScannerDynamicUi() {
    ...
    if (collected) {
        collected.innerHTML = state.scanCodes.length ? state.scanCodes.map((code) => chip(`${escapeHtml(code)} <button type="button" class="elchipbtn" data-a="remove-scanned-code" data-id="${escapeHtml(code)}">Ã—</button>`, 'bg-indigo-100 text-indigo-700')).join(' ') : '<span class="text-sm text-gray-400">Noch keine Codes erfasst.</span>';
    }
...
function renderListSelect() {
    return `<div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-3"><div class="text-xs font-bold tracking-wide text-slate-500 whitespace-nowrap">Liste wechseln:</div><div class="w-full sm:w-1/2"><select id="el-list-select" class="els text-center" ${state.lists.length ? '' : 'disabled'}>${state.lists.length ? state.lists.map((l) => `<option value="${l.id}" ${state.listId === l.id ? 'selected' : ''}>${escapeHtml(l.name || 'Liste')}</option>`).join('') : '<option value="">Keine Liste verfÃ¼gbar</option>'}</select></div></div>`;
}

function renderDetailRows(list) {
    if (!state.detailsOpen) return '';
    const activityText = state.activity && Date.now() - (toDate(state.activity.createdAt)?.getTime() || 0) <= ACTIVITY_MS ? `${escapeHtml(state.activity.actorName || 'User')} Â· ${dt(state.activity.createdAt)} Â· ${escapeHtml(state.activity.text || '')}` : 'Keine aktuelle AktivitÃ¤t.';
    return `<div class="space-y-3"><div>${renderListSelect()}</div><div class="flex flex-wrap justify-between gap-2 items-start"><div class="flex flex-wrap gap-2 text-xs">${list ? `${list.ownerId === uid() ? chip('Owner', 'bg-indigo-100 text-indigo-700') : chip(permActive(perm()) ? 'freigegeben' : 'abgelaufen', permActive(perm()) ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700')} ${chip(`Letztes Update ${dt(list.updatedAt)}`)} ${chip(`Von ${escapeHtml(list.updatedByName || list.ownerName || 'â€”')}`)}` : '<span class="text-sm text-gray-500">Bitte zuerst eine Liste auswÃ¤hlen.</span>'}</div><div class="flex flex-wrap gap-2">${list ? `<button class="elb bg-gray-100 text-gray-700" data-a="store-display">${state.storeDisplay === 'split' ? 'Nach GeschÃ¤ft' : 'Kombiniert'}</button>` : ''}</div></div><div class="flex flex-wrap justify-between gap-2 items-center"><label class="inline-flex items-center gap-2 text-xs font-bold text-gray-600"><input id="el-store-numbers" type="checkbox" ${state.storeNumbers ? 'checked' : ''}> GeschÃ¤fte nummerieren</label><div class="text-xs text-gray-400">${list ? `${state.stores.length} GeschÃ¤fte verfÃ¼gbar` : 'Keine Liste ausgewÃ¤hlt.'}</div></div><div class="space-y-2 text-xs text-gray-600"><div>${list ? (state.presence.length ? state.presence.map((p) => chip(`${escapeHtml(p.userName || p.userId)} (${escapeHtml(p.currentArea || 'Liste')})`, 'bg-emerald-100 text-emerald-700')).join(' ') : '<span class="text-gray-400">Niemand sonst gerade aktiv.</span>') : '<span class="text-gray-400">Keine Liste ausgewÃ¤hlt.</span>'}</div><div class="font-semibold text-gray-600">${activityText}</div></div></div>`;
}
...
function renderHeaderActive(list) {
    ...
    return `<div class="space-y-3"><div class="border-b border-slate-200 pb-3"><div class="flex items-center gap-2"><button class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-black text-slate-700 shadow-sm" data-a="back-home">â† ZurÃ¼ck</button><div class="min-w-0 flex-1 flex justify-center"><div class="inline-flex max-w-full items-center rounded-full border-2 px-4 py-2 text-sm font-black shadow-sm ${listBoxClass}">${escapeHtml(listBoxText)}</div></div><div class="flex items-center gap-2"><button class="elb bg-gray-100 text-gray-700" data-a="open-mode-picker">${escapeHtml(currentModeDef().label)}</button><button class="elb bg-gray-100 text-gray-700 !px-3" data-a="toggle-details" title="MenÃ¼">â‹®</button></div></div></div>${state.detailsOpen ? `<div class="elc">${renderDetailRows(list)}</div>` : ''}${renderActionBar()}</div>`;
}
...
function selectList(nextId) {
    ...
    if (list.ownerId !== uid() && !permActive(perm(list.id))) {
        alertUser('Diese Liste ist auÃŸerhalb deiner Zugriffszeit gesperrt.', 'error');
        return;
    }
    state.listId = nextId;
    persistListId();
...
function render() {
    ...
    main.innerHTML = `
        ...
        ${blocked ? '<div class="elc text-sm text-red-700">Diese Liste ist sichtbar, aber auÃŸerhalb deiner Zugriffszeit aktuell gesperrt.</div>' : renderBodyActive()}
...
function renderBody() {
    const list = activeList();
    if (!list) return '<div class="elc text-sm text-gray-500">Bitte zuerst eine Liste auswÃ¤hlen.</div>';
    if (state.mode === 'shop') {
        return `<div class="space-y-3"><div class="elc"><div class="grid grid-cols-[90px_110px_minmax(0,1fr)] gap-2"><input id="el-q" class="eli" value="${escapeHtml(state.q)}"><select id="el-unit" class="els">${UNITS.map((u) => `<option value="${u}" ${state.unit === u ? 'selected' : ''}>${u}</option>`).join('')}</select><input id="el-title" class="eli" placeholder="Artikel eingeben..." value="${escapeHtml(state.title)}"></div><div class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]"><select id="el-store-add" class="els"><option value="">GeschÃ¤ft zuordnen...</option>${state.stores.map((s) => `<option value="${s.id}">${escapeHtml(s.name)}</option>`).join('')}</select><input id="el-note" class="eli" placeholder="Anmerkung optional" value="${escapeHtml(state.note)}"><button class="elb a" data-a="add-item" ${!canAdd() ? 'disabled' : ''}>+ HinzufÃ¼gen</button></div>${state.storeIds.length ? `<div class="elm">${state.storeIds.map((id) => chip(`${escapeHtml(state.stores.find((s) => s.id === id)?.name || id)} <button data-a="del-store" data-id="${id}">Ã—</button>`, 'bg-orange-100 text-orange-700')).join(' ')}</div>` : '<div class="text-xs text-gray-400">Optional einem oder mehreren GeschÃ¤ften zuordnen.</div>'}</div>`;
    }
    ...
    if (!canManage()) return '<div class="elc text-sm text-red-700">Keine Verwaltungsberechtigung fÃ¼r diese Liste.</div>';
    if (state.section === 'general') return `<div class="space-y-3"><div class="elc"><div class="elstat"><div><div class="text-[11px] font-bold uppercase text-gray-500">Aktive Listen</div><div class="text-xl font-black text-indigo-700">${state.lists.filter((l) => l.active !== false).length}</div></div><div><div class="text-[11px] font-bold uppercase text-gray-500">Offene Artikel</div><div class="text-xl font-black text-orange-700">${state.items.filter((x) => x.status !== 'checked').length}</div></div><div><div class="text-[11px] font-bold uppercase text-gray-500">Letztes Update</div><div class="text-sm font-black text-slate-700">${dt(activeList()?.updatedAt)}</div></div></div></div>${state.lists.map((l) => `<div class="elc"><div class="flex justify-between gap-2"><div><div class="font-bold text-sm">${escapeHtml(l.name)}</div><div class="text-xs text-gray-500">${dt(l.updatedAt)} Â· ${escapeHtml(l.updatedByName || l.ownerName || 'â€”')}</div></div><div class="text-xs font-bold text-gray-600">${l.id === state.listId ? state.items.filter((x) => x.status !== 'checked').length : 'â€¦'} offen</div></div></div>`).join('')}</div>`;
    if (state.section === 'stores') return `<div class="space-y-3"><div class="elc flex flex-wrap gap-2"><input id="el-draft-store" class="eli" placeholder="Neues GeschÃ¤ft" value="${escapeHtml(state.drafts.store)}"><button class="elb a" data-a="add-store" ${!canManageWrite() ? 'disabled' : ''}>+ GeschÃ¤ft</button></div>${(activeList()?.storeOrder?.length ? activeList().storeOrder : state.stores.map((s) => s.id)).map((id, i, arr) => { const s = state.stores.find((x) => x.id === id); if (!s) return ''; const categoryNames = (s.categoryOrder || []).map((catId) => state.categories.find((c) => c.id === catId)?.name).filter(Boolean); return `<div class="elc space-y-2"><div class="flex flex-wrap justify-between gap-2 items-start"><div><div class="font-bold text-sm">${escapeHtml(s.name)}</div><div class="text-xs text-gray-500">${categoryNames.length ? `${categoryNames.length} Kategorie(n) fÃ¼r Sortierung gewÃ¤hlt` : 'Noch keine Kategorien ausgewÃ¤hlt'}</div></div><div class="flex flex-wrap gap-2"><button class="elb bg-gray-100 text-gray-700" data-a="store-up" data-id="${s.id}" ${i === 0 || !canManageWrite() ? 'disabled' : ''}>â†‘</button><button class="elb bg-gray-100 text-gray-700" data-a="store-down" data-id="${s.id}" ${i === arr.length - 1 || !canManageWrite() ? 'disabled' : ''}>â†“</button><button class="elb bg-indigo-100 text-indigo-700" data-a="open-store-categories" data-id="${s.id}" ${!canManageWrite() ? 'disabled' : ''}>Kategorienwartung</button><button class="elb bg-red-600 text-white" data-a="del-store-master" data-id="${s.id}" ${!canManageWrite() ? 'disabled' : ''}>LÃ¶schen</button></div></div><div class="elm">${categoryNames.length ? categoryNames.map((name) => chip(escapeHtml(name), 'bg-indigo-100 text-indigo-700')).join(' ') : '<span class="text-xs text-gray-400">Keine Kategorien sortiert.</span>'}</div></div>`; }).join('')}</div>`;
    if (state.section === 'articles') return `<div class="space-y-3"><div class="elc flex flex-wrap gap-2"><input id="el-article-search" class="eli" placeholder="Artikel suchen..." value="${escapeHtml(state.articleSearch)}"><label class="inline-flex items-center gap-1 rounded-full px-2 py-1 text-[11px] font-bold bg-slate-100 text-slate-700"><input type="checkbox" id="el-missing-ean" ${state.missingEanOnly ? 'checked' : ''}> Ohne EAN</label><button class="elb a" data-a="open-article" ${!canManageWrite() ? 'disabled' : ''}>+ Artikel</button></div>${filteredManageArticles().map((a) => { const hasAnyEan = !!(a.eanCodes?.length || a.variants?.some((v) => v?.eanCodes?.length)); return `<div class="elc space-y-2"><div class="flex flex-wrap justify-between gap-2 items-center"><div><div class="font-bold text-sm">${escapeHtml(a.title)}</div><div class="text-xs text-gray-500">${fmtQty(a.defaultQuantity || 1)} ${escapeHtml(a.defaultUnit || 'StÃ¼ck')} Â· ${escapeHtml(state.categories.find((c) => c.id === a.categoryId)?.name || 'Ohne Kategorie')}</div></div><div class="flex flex-wrap gap-2">${hasAnyEan ? chip('EAN OK', 'bg-emerald-100 text-emerald-700') : `<button class="elb bg-red-100 text-red-700" data-a="capture-ean" data-id="${a.id}" ${!canManageWrite() ? 'disabled' : ''}>ohne EAN</button>`}<button class="elb bg-gray-100 text-gray-700" data-a="edit-article" data-id="${a.id}" ${!canManageWrite() ? 'disabled' : ''}>Bearbeiten</button></div></div><div class="text-xs text-gray-600">${escapeHtml((a.persistentNotes || []).join(' Â· ')) || '<span class="text-gray-400">Keine permanente Anmerkung.</span>'}</div><div class="elm">${(a.eanCodes || []).length ? (a.eanCodes || []).map((code) => chip(escapeHtml(code))).join(' ') : '<span class="text-xs text-gray-400">Keine Haupt-EAN</span>'}</div></div>`; }).join('') || '<div class="elc text-sm text-gray-400">Keine Artikel gefunden.</div>'}</div>`;
    if (state.section === 'categories') return `<div class="space-y-3"><div class="elc flex flex-wrap gap-2"><input id="el-draft-category" class="eli" placeholder="Neue Kategorie" value="${escapeHtml(state.drafts.category)}"><button class="elb a" data-a="add-category" ${!canManageWrite() ? 'disabled' : ''}>+ Kategorie</button></div>${state.categories.map((c) => `<div class="elc flex flex-wrap justify-between gap-2"><div class="font-bold text-sm">${escapeHtml(c.name)}</div><button class="elb bg-red-600 text-white" data-a="del-category" data-id="${c.id}" ${!canManageWrite() ? 'disabled' : ''}>LÃ¶schen</button></div>`).join('')}</div>`;
    if (state.section === 'remarks') return `<div class="space-y-3"><div class="elc flex flex-wrap gap-2"><input id="el-draft-remark" class="eli" placeholder="HÃ¤ufige Anmerkung" value="${escapeHtml(state.drafts.remark)}"><button class="elb a" data-a="add-remark" ${!canManageWrite() ? 'disabled' : ''}>+ Anmerkung</button></div>${state.remarks.map((n) => `<div class="elc flex flex-wrap justify-between gap-2"><div class="font-bold text-sm">${escapeHtml(n.text || n.name || '')}</div><button class="elb bg-red-600 text-white" data-a="del-remark" data-id="${n.id}" ${!canManageWrite() ? 'disabled' : ''}>LÃ¶schen</button></div>`).join('') || '<div class="elc text-sm text-gray-400">Keine Anmerkungen.</div>'}</div>`;
    return `<div class="space-y-3"><div class="elc flex flex-wrap gap-2"><input id="el-draft-note" class="eli" placeholder="Meta-Notiz" value="${escapeHtml(state.drafts.note)}"><button class="elb a" data-a="add-note" ${!canManageWrite() ? 'disabled' : ''}>+ Notiz</button></div>${state.notes.map((n) => `<div class="elc flex flex-wrap justify-between gap-2"><div class="font-bold text-sm">${escapeHtml(n.text || n.name || '')}</div><button class="elb bg-red-600 text-white" data-a="del-note" data-id="${n.id}" ${!canManageWrite() ? 'disabled' : ''}>LÃ¶schen</button></div>`).join('') || '<div class="elc text-sm text-gray-400">Keine Notizen.</div>'}</div>`;
}
...
function renderBodyActive() {
    ...
    if (state.section === 'stores') {
        return `<div class="space-y-3"><div class="elc flex flex-wrap gap-2"><input id="el-draft-store" class="eli" placeholder="Neues GeschÃ¤ft" value="${escapeHtml(state.drafts.store)}"><button class="elb a" data-a="add-store" ${!canManageWrite() ? 'disabled' : ''}>+ GeschÃ¤ft</button></div>${(activeList()?.storeOrder?.length ? activeList().storeOrder : state.stores.map((s) => s.id)).map((id, i, arr) => { const s = state.stores.find((x) => x.id === id); if (!s) return ''; const categoryNames = (s.categoryOrder || []).map((catId) => state.categories.find((c) => c.id === catId)?.name).filter(Boolean); return `<div class="elc space-y-2"><div class="flex flex-wrap justify-between gap-2 items-start"><div><div class="font-bold text-sm">${escapeHtml(s.name)}</div><div class="text-xs text-gray-500">${categoryNames.length ? `${categoryNames.length} Kategorie(n) fÃ¼r Sortierung gewÃ¤hlt` : 'Noch keine Kategorien ausgewÃ¤hlt'}</div></div><div class="flex flex-wrap gap-2"><button class="elb bg-gray-100 text-gray-700" data-a="store-up" data-id="${s.id}" ${i === 0 || !canManageWrite() ? 'disabled' : ''}>â†‘</button><button class="elb bg-gray-100 text-gray-700" data-a="store-down" data-id="${s.id}" ${i === arr.length - 1 || !canManageWrite() ? 'disabled' : ''}>â†“</button><button class="elb bg-indigo-100 text-indigo-700" data-a="open-store-categories" data-id="${s.id}" ${!canManageWrite() ? 'disabled' : ''}>Kategorienwartung</button><button class="elb bg-red-600 text-white" data-a="del-store-master" data-id="${s.id}" ${!canManageWrite() ? 'disabled' : ''}>LÃ¶schen</button></div></div><div class="flex flex-wrap items-center gap-1.5 text-xs">${categoryNames.length ? categoryNames.map((name, index) => `${index ? '<span class="font-black text-slate-400">â†’</span>' : ''}${chip(escapeHtml(name), 'bg-indigo-100 text-indigo-700')}`).join(' ') : '<span class="text-xs text-gray-400">Keine Kategorien sortiert.</span>'}</div></div>`; }).join('')}</div>`;
    }
    return renderBody();
}
...
function renderStoreCategoryEditor() {
    ...
    el.innerHTML = `<div class="elpanel p-4 sm:p-5 space-y-4"><div class="flex flex-wrap justify-between gap-2 items-center"><div><div class="text-xl font-black text-gray-900">Kategorienwartung</div><div class="text-sm text-gray-500">${escapeHtml(store?.name || 'GeschÃ¤ft')} sortiert die Anzeige im Einkaufsmodus.</div></div><button class="elb bg-gray-100 text-gray-700" data-a="close-store-categories">SchlieÃŸen</button></div><div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]"><div class="elc space-y-2"><div class="text-sm font-black text-gray-900">VerfÃ¼gbare Kategorien</div>${available.length ? available.map((c) => `<label class="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700"><input type="checkbox" data-a="toggle-store-category" data-id="${c.id}"> ${escapeHtml(c.name)}</label>`).join('') : '<div class="text-sm text-gray-400">Alle Kategorien sind bereits ausgewÃ¤hlt.</div>'}</div><div class="elc space-y-2"><div class="flex items-center justify-between gap-2"><div class="text-sm font-black text-gray-900">AusgewÃ¤hlte Reihenfolge</div><div class="text-xs font-semibold text-slate-400">Ziehen zum Sortieren</div></div>${selected.length ? selected.map((c, index) => `<div class="elstorecat-row flex items-center justify-between gap-2 rounded-2xl border border-indigo-100 bg-indigo-50 px-3 py-2"><div class="flex items-center gap-3 min-w-0"><button type="button" class="cursor-grab rounded-xl border border-indigo-200 bg-white px-2 py-1 text-base font-black text-indigo-700" data-drag-store-category data-id="${c.id}" aria-label="Kategorie ziehen">â†•</button><div class="text-sm font-bold text-indigo-900">${index + 1}. ${escapeHtml(c.name)}</div></div><div class="flex gap-2"><button class="elb bg-white text-gray-700" data-a="store-category-up" data-id="${c.id}" ${index === 0 ? 'disabled' : ''}>â†‘</button><button class="elb bg-white text-gray-700" data-a="store-category-down" data-id="${c.id}" ${index === selected.length - 1 ? 'disabled' : ''}>â†“</button><button class="elb bg-red-100 text-red-700" data-a="toggle-store-category" data-id="${c.id}">Entfernen</button></div></div>`).join('') : '<div class="text-sm text-gray-400">Noch keine Kategorie gewÃ¤hlt.</div>'}</div></div><div class="flex flex-wrap justify-end gap-2"><button class="elb bg-gray-100 text-gray-700" data-a="close-store-categories">Abbruch</button><button class="elb bg-emerald-600 text-white" data-a="save-store-categories">Speichern</button></div></div>`;
}
...
function renderStoreCategoryEditorActive() {
    ...
    el.innerHTML = `<div class="elpanel p-4 sm:p-5 space-y-4"><div class="flex flex-wrap justify-between gap-2 items-center"><div><div class="text-xl font-black text-gray-900">Kategorienwartung</div><div class="text-sm text-gray-500">${escapeHtml(store?.name || 'GeschÃ¤ft')} sortiert die Anzeige im Einkaufsmodus.</div></div><button class="elb bg-gray-100 text-gray-700" data-a="close-store-categories">SchlieÃŸen</button></div><div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]"><div class="elc space-y-2"><div class="text-sm font-black text-gray-900">VerfÃ¼gbare Kategorien</div>${available.length ? available.map((c) => `<label class="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700"><input type="checkbox" data-a="toggle-store-category" data-id="${c.id}"> ${escapeHtml(c.name)}</label>`).join('') : '<div class="text-sm text-gray-400">Alle Kategorien sind bereits ausgewÃ¤hlt.</div>'}</div><div class="elc space-y-2"><div class="flex items-center justify-between gap-2"><div class="text-sm font-black text-gray-900">AusgewÃ¤hlte Reihenfolge</div><div class="text-xs font-semibold text-slate-400">Ziehen zum Sortieren</div></div>${selected.length ? selected.map((c, index) => { const dropClass = drag?.overId === c.id ? (drag.after ? ' drop-after' : ' drop-before') : ''; const draggingClass = drag?.id === c.id ? ' dragging' : ''; return `<div class="elstorecat-row flex items-center justify-between gap-2 rounded-2xl border border-indigo-100 bg-indigo-50 px-3 py-2${dropClass}${draggingClass}" data-store-category-row data-id="${c.id}"><div class="flex items-center gap-3 min-w-0"><button type="button" class="cursor-grab rounded-xl border border-indigo-200 bg-white px-2 py-1 text-base font-black text-indigo-700" data-drag-store-category data-id="${c.id}" aria-label="Kategorie ziehen">â†•</button><div class="text-sm font-bold text-indigo-900">${index + 1}. ${escapeHtml(c.name)}</div></div><div class="flex gap-2"><button class="elb bg-white text-gray-700" data-a="store-category-up" data-id="${c.id}" ${index === 0 ? 'disabled' : ''}>â†‘</button><button class="elb bg-white text-gray-700" data-a="store-category-down" data-id="${c.id}" ${index === selected.length - 1 ? 'disabled' : ''}>â†“</button><button class="elb bg-red-100 text-red-700" data-a="toggle-store-category" data-id="${c.id}">Entfernen</button></div></div>`; }).join('') : '<div class="text-sm text-gray-400">Noch keine Kategorie gewÃ¤hlt.</div>'}</div></div><div class="flex flex-wrap justify-end gap-2"><button class="elb bg-gray-100 text-gray-700" data-a="close-store-categories">Abbruch</button><button class="elb bg-emerald-600 text-white" data-a="save-store-categories">Speichern</button></div></div>`;
}
...
function renderModePicker() {
    ...
    el.innerHTML = `<div class="elpanel p-4 sm:p-5 space-y-4"><div class="flex flex-wrap justify-between gap-2 items-center"><div><div class="text-xl font-black text-gray-900">Modus wÃ¤hlen</div><div class="text-sm text-gray-500">Bitte den gewÃ¼nschten Arbeitsmodus auswÃ¤hlen.</div></div><button class="elb bg-gray-100 text-gray-700" data-a="close-mode-picker">SchlieÃŸen</button></div><div class="space-y-2">${MODES.map((m) => `<button class="w-full rounded-2xl border p-3 text-left ${state.mode === m.id ? 'border-indigo-300 ring-2 ring-indigo-500 bg-indigo-50' : 'border-gray-200 bg-white'}" data-a="mode" data-v="${m.id}"><div class="font-bold text-sm">${m.label}</div></button>`).join('')}</div></div>`;
}
...
function renderSettings() {
    ...
    el.innerHTML = `<div class="elpanel p-4 sm:p-5 space-y-4"><div class="flex flex-wrap justify-between gap-2 items-center"><div><div class="text-xl font-black text-gray-900">Einkaufslisten verwalten</div><div class="text-sm text-gray-500">Privatliste ist fix, nicht umbenennbar und nicht teilbar.</div></div><button class="elb bg-gray-100 text-gray-700" data-a="close-settings">SchlieÃŸen</button></div><div class="grid gap-4 lg:grid-cols-[320px_minmax(0,1fr)]"><div class="elc space-y-3"><div class="flex flex-wrap justify-between gap-2 items-center"><div class="font-black text-sm">Listen</div><button class="elb a" data-a="create-list" ${!(currentUser.permissions || []).includes('EINKAUFSLISTE_CREATE') && currentUser.role !== 'SYSTEMADMIN' ? 'disabled' : ''}>+ Liste</button></div>${state.lists.map((l) => `<button class="w-full text-left rounded-2xl border p-3 ${state.listId === l.id ? 'ring-2 ring-indigo-500 border-indigo-300' : 'border-gray-200'}" data-a="list" data-id="${l.id}"><div class="font-bold text-sm">${escapeHtml(l.name)}</div><div class="text-xs text-gray-500">${l.isPrivateSystemList ? 'Privatliste' : 'Normale Liste'} Â· ${l.active !== false ? 'aktiv' : 'pausiert'}</div></button>`).join('')}</div><div class="space-y-4">${list ? `<div class="elc space-y-3"><div class="font-black text-sm">Allgemein</div><div class="grid gap-3 sm:grid-cols-2"><input id="el-set-name" class="eli" value="${escapeHtml(list.name || '')}" ${list.isPrivateSystemList || list.ownerId !== uid() ? 'disabled' : ''}><label class="inline-flex items-center gap-2 text-sm font-bold text-gray-700"><input type="checkbox" id="el-set-active" ${list.active !== false ? 'checked' : ''} ${list.isPrivateSystemList || list.ownerId !== uid() ? 'disabled' : ''}> aktiv</label></div>${list.ownerId === uid() && !list.isPrivateSystemList ? `<div class="flex flex-wrap gap-2"><button class="elb a" data-a="save-list">Liste speichern</button><button class="elb bg-red-600 text-white" data-a="delete-list" data-id="${list.id}">Liste lÃ¶schen</button></div>` : '<div class="text-sm text-gray-400">Nur der Listeninhaber kann diese Liste Ã¤ndern.</div>'}</div><div class="elc space-y-3"><div class="font-black text-sm">Freigaben</div><div class="grid gap-2 sm:grid-cols-2"><select id="el-c-user" class="els"><option value="">Person wÃ¤hlen...</option>${candidates.map((u) => `<option value="${u.id}" ${state.collab.userId === u.id ? 'selected' : ''}>${escapeHtml(u.name)}</option>`).join('')}</select><input id="el-c-from" type="datetime-local" class="eli" value="${escapeHtml(state.collab.accessFrom || dtLocal(new Date()))}"><input id="el-c-until" type="datetime-local" class="eli" value="${escapeHtml(state.collab.accessUntil || '')}"><div class="elm"><label class="inline-flex items-center gap-1 text-xs font-bold text-gray-600"><input id="el-cr" type="checkbox" ${state.collab.canRead ? 'checked' : ''}> Lesen</label><label class="inline-flex items-center gap-1 text-xs font-bold text-gray-600"><input id="el-ca" type="checkbox" ${state.collab.canAdd ? 'checked' : ''}> HinzufÃ¼gen</label><label class="inline-flex items-center gap-1 text-xs font-bold text-gray-600"><input id="el-cs" type="checkbox" ${state.collab.canShop ? 'checked' : ''}> Einkaufen</label><label class="inline-flex items-center gap-1 text-xs font-bold text-gray-600"><input id="el-cm" type="checkbox" ${state.collab.canManage ? 'checked' : ''}> Verwalten</label><label class="inline-flex items-center gap-1 text-xs font-bold text-gray-600"><input id="el-cw" type="checkbox" ${state.collab.canManageWrite ? 'checked' : ''}> Schreibrechte</label></div></div><button class="elb a" data-a="save-collab" ${list.ownerId !== uid() || list.isPrivateSystemList ? 'disabled' : ''}>Freigabe speichern</button>${permissionEntries.length ? permissionEntries.map((p) => `<div class="rounded-2xl border border-gray-200 p-3"><div class="flex flex-wrap justify-between gap-2 items-center"><div><div class="font-bold text-sm">${escapeHtml(p.userName || p.userId)}</div><div class="text-xs text-gray-500">${dt(p.accessFrom)} bis ${p.accessUntil ? dt(p.accessUntil) : 'offen'}</div></div><button class="elb bg-red-600 text-white" data-a="del-collab" data-id="${p.userId}" ${list.ownerId !== uid() || list.isPrivateSystemList ? 'disabled' : ''}>Entfernen</button></div></div>`).join('') : '<div class="text-sm text-gray-400">Keine Freigaben.</div>'}</div>` : '<div class="elc text-sm text-gray-400">Bitte zuerst eine Liste wÃ¤hlen.</div>'}</div></div>`;
}
...
function renderPurchase() {
    ...
    el.innerHTML = `<div class="elpanel p-4 sm:p-5 space-y-4"><div class="space-y-1 text-center"><div class="text-xl font-black text-gray-900">${escapeHtml(p.title)}</div><div class="text-sm text-gray-500">${p.kind === 'scan' ? 'Scan-Erfassung' : 'MengenÃ¼bernahme'}</div></div><div class="elc space-y-3 bg-slate-50"><div class="text-xs font-bold uppercase text-gray-500 text-center">Menge</div><div class="flex min-h-[6.5rem] flex-col items-center justify-center text-center"><div class="text-5xl font-black leading-none text-indigo-700">${purchaseDisplayValue(p)}</div><div class="mt-2 text-base font-bold text-slate-500">${escapeHtml(p.unit)}</div></div>${p.target ? `<div class="text-sm text-center text-gray-600">Soll lt. Liste: ${fmtQty(p.target)} ${escapeHtml(p.unit)}</div>` : ''}${delta !== 0 ? `<div class="rounded-xl border border-orange-300 bg-orange-50 px-3 py-2 text-sm font-bold text-orange-800 text-center">${delta > 0 ? `Achtung +${fmtQty(delta)} â€“ Ã¼bernehmen?` : `Achtung ${fmtQty(delta)} â€“ Ã¼bernehmen?`}</div>` : ''}</div><div class="space-y-2"><div class="grid grid-cols-4 gap-2"><button class="${keyBtn}" data-a="digit" data-v="7">7</button><button class="${keyBtn}" data-a="digit" data-v="8">8</button><button class="${keyBtn}" data-a="digit" data-v="9">9</button><button class="${keyAccent} border-indigo-200 bg-indigo-50 text-indigo-700" data-a="full">SOLL</button></div><div class="grid grid-cols-4 gap-2"><button class="${keyBtn}" data-a="digit" data-v="4">4</button><button class="${keyBtn}" data-a="digit" data-v="5">5</button><button class="${keyBtn}" data-a="digit" data-v="6">6</button><button class="${keyAccent} border-orange-200 bg-orange-50 text-orange-700" data-a="clear">C</button></div><div class="grid grid-cols-4 gap-2"><button class="${keyBtn}" data-a="digit" data-v="1">1</button><button class="${keyBtn}" data-a="digit" data-v="2">2</button><button class="${keyBtn}" data-a="digit" data-v="3">3</button><button class="${keyBtn}" data-a="back">âŒ«</button></div><div class="grid grid-cols-[minmax(0,1.25fr)_minmax(0,1fr)_minmax(0,1fr)] gap-2"><div class="grid grid-cols-[7fr_3fr] gap-2"><button class="${keyAccent} border-slate-300 bg-slate-100 text-slate-700" data-a="close-purchase">Abbruch</button><button class="${keyBtn}" data-a="digit" data-v=",">,</button></div><button class="${keyBtn}" data-a="digit" data-v="0">0</button><button class="${keyAccent} border-emerald-600 bg-emerald-600 text-white" data-a="confirm-purchase">Ãœbernehmen</button></div></div></div>`;
}
...
function renderDetail() {
    ...
    el.innerHTML = `<div class="elpanel p-4 sm:p-5 space-y-4"><div class="flex flex-wrap justify-between gap-2 items-center"><div><div class="text-xl font-black text-gray-900">Produkt bearbeiten</div><div class="text-sm text-gray-500">Langer Klick auf den Namen Ã¶ffnet dieses Fenster.</div></div><button class="elb bg-gray-100 text-gray-700" data-a="close-detail">SchlieÃŸen</button></div><div class="grid gap-3 sm:grid-cols-2"><input id="el-d-title" class="eli" value="${escapeHtml(item.title || '')}" ${!canEditItems() ? 'disabled' : ''}><input id="el-d-qty" class="eli" value="${escapeHtml(formatEditableQty(item.quantity || '1'))}" ${!canEditItems() ? 'disabled' : ''}></div><div class="grid gap-3 sm:grid-cols-2"><select id="el-d-cat" class="els" ${!canEditItems() ? 'disabled' : ''}><option value="">Kategorie wÃ¤hlen...</option>${state.categories.map((c) => `<option value="${c.id}" ${item.categoryId === c.id ? 'selected' : ''}>${escapeHtml(c.name)}</option>`).join('')}</select><div class="flex items-center rounded-2xl border border-gray-200 bg-slate-50 px-3 text-sm font-bold text-gray-600">Einheit: ${escapeHtml(item.unit || 'StÃ¼ck')}</div></div><textarea id="el-d-note" class="elt" placeholder="Anmerkung fÃ¼r diese Position" ${!canEditItems() ? 'disabled' : ''}>${escapeHtml(item.note || '')}</textarea><textarea id="el-d-pnote" class="elt" placeholder="Gespeicherte Anmerkung" ${!canEditItems() ? 'disabled' : ''}>${escapeHtml(item.persistentNote || '')}</textarea><div class="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]"><select id="el-d-move-list" class="els" ${!otherLists.length || !canEditItems() ? 'disabled' : ''}><option value="">Auf andere Liste verschieben...</option>${otherLists.map((list) => `<option value="${list.id}">${escapeHtml(list.name)}</option>`).join('')}</select><button class="elb bg-gray-100 text-gray-700" data-a="move-detail" ${!otherLists.length || !canEditItems() ? 'disabled' : ''}>Verschieben</button></div><div class="elc space-y-2 bg-slate-50 text-sm"><div><b>HinzugefÃ¼gt:</b> ${dt(item.createdAt)} von ${escapeHtml(item.createdByName || item.createdBy || 'â€”')}</div>${item.checkedAt ? `<div><b>Abgehakt:</b> ${dt(item.checkedAt)} von ${escapeHtml(item.checkedByName || item.checkedBy || 'â€”')}</div>` : ''}${item.restoredAt ? `<div><b>Wiederhergestellt:</b> ${dt(item.restoredAt)} von ${escapeHtml(item.restoredByName || item.restoredBy || 'â€”')}</div>` : ''}<div><b>EAN-Codes:</b> ${escapeHtml((article?.eanCodes || []).join(', ') || 'keine')}</div></div><div class="flex flex-wrap justify-between gap-2"><button class="elb bg-red-600 text-white" data-a="delete-detail-item" ${!canEditItems() ? 'disabled' : ''}>LÃ¶schen</button><div class="flex flex-wrap gap-2"><button class="elb bg-gray-100 text-gray-700" data-a="close-detail">SchlieÃŸen</button><button class="elb a" data-a="save-detail" ${!canEditItems() ? 'disabled' : ''}>Speichern</button></div></div></div>`;
}

function renderArticle() {
    const el = document.getElementById('el-article'); if (!el) return; const a = state.articleEditor; el.className = `elmodal ${a ? 'o' : ''}`; if (!a) { el.innerHTML = ''; return; } el.innerHTML = `<div class="elpanel p-4 sm:p-5 space-y-4"><div class="flex flex-wrap justify-between gap-2 items-center"><div class="text-xl font-black text-gray-900">${a.id ? 'Artikel bearbeiten' : 'Artikel anlegen'}</div><button class="elb bg-gray-100 text-gray-700" data-a="close-article">SchlieÃŸen</button></div><div class="grid gap-2 sm:grid-cols-2"><input id="ela-title" class="eli" placeholder="Bezeichnung" value="${escapeHtml(a.title || '')}"><input id="ela-q" class="eli" placeholder="Standardmenge" value="${escapeHtml(String(a.defaultQuantity || '1'))}"><select id="ela-unit" class="els">${UNITS.map((u) => `<option value="${u}" ${a.defaultUnit === u ? 'selected' : ''}>${u}</option>`).join('')}</select><select id="ela-cat" class="els"><option value="">Kategorie wÃ¤hlen...</option>${state.categories.map((c) => `<option value="${c.id}" ${a.categoryId === c.id ? 'selected' : ''}>${escapeHtml(c.name)}</option>`).join('')}</select></div><textarea id="ela-ean" class="elt" placeholder="EAN-Codes, eine Zeile pro Code">${escapeHtml((a.eanCodes || []).join('\n'))}</textarea>${a.id ? `<div class="flex justify-end"><button class="elb bg-red-100 text-red-700" data-a="capture-ean" data-id="${a.id}">Ohne EAN scannen</button></div>` : ''}<textarea id="ela-var" class="elt" placeholder="Varianten je Zeile: Name|EAN1,EAN2">${escapeHtml((a.variants || []).map((v) => `${v.label || ''}|${(v.eanCodes || []).join(',')}`).join('\n'))}</textarea><textarea id="ela-note" class="elt" placeholder="Permanente Anmerkungen, je Zeile">${escapeHtml((a.persistentNotes || []).join('\n'))}</textarea><div class="elm">${state.stores.map((s) => `<label class="inline-flex items-center gap-1 rounded-full px-2 py-1 text-[11px] font-bold ${(a.storeIds || []).includes(s.id) ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-700'}"><input type="checkbox" data-a="art-store" data-id="${s.id}" ${(a.storeIds || []).includes(s.id) ? 'checked' : ''}> ${escapeHtml(s.name)}</label>`).join(' ')}</div><div class="flex flex-wrap justify-end gap-2">${a.id ? `<button class="elb bg-red-600 text-white" data-a="delete-article" data-id="${a.id}">LÃ¶schen</button>` : ''}<button class="elb bg-emerald-600 text-white" data-a="save-article">Speichern</button></div></div>`;
}

function renderScanner() {
    const el = document.getElementById('el-scanner'); if (!el) return; el.className = `elmodal ${state.scanOpen ? 'o' : ''}`; if (!state.scanOpen) { el.innerHTML = ''; stopScanner(); return; } el.innerHTML = `<div class="elpanel p-4 sm:p-5 space-y-4"><div class="flex flex-wrap justify-between gap-2 items-center"><div><div class="text-xl font-black text-gray-900">Scanner</div><div class="text-sm text-gray-500">Barcode + QR live, ohne Refresh.</div></div><button class="elb bg-gray-100 text-gray-700" data-a="close-scan">SchlieÃŸen</button></div><div class="elcam"><video id="el-video" autoplay playsinline muted></video></div><div class="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto]"><input id="el-scan-manual" class="eli" placeholder="EAN/QR manuell eingeben"><button class="elb a" data-a="manual-scan">Code Ã¼bernehmen</button></div><div id="el-scan-status" class="text-sm text-gray-500">Kamera wird gestartet...</div></div>`; startScanner();
}
...
function onKeyDownActive(e) {
 ...
 function onHold(payload) {
     ...
     holdConsumedAt = Date.now();
     if (payload.action === 'edit-item') {
...
async function addItem() {
    if (!canAdd()) return alertUser('Keine Berechtigung zum HinzufÃ¼gen.', 'error');
    ...
    const persistentNote = article.persistentNotes?.length ? article.persistentNotes.join(' Â· ') : '';
    ...
    await logActivity('Artikel hinzugefÃ¼gt', { title, quantity, unit: state.unit });
...
async function handleDouble(id, action) {
    ...
    if (now - last > DOUBLE_MS) {
        if (action === 'check') alertUser('Abhaken: Schneller Doppelklick/Mengeneingabe: Langer Klick', 'info');
        return;
    }
    if (action === 'check') { await checkItemDirect(id); return; }
...
async function checkItemDirect(id) {
    ...
    await logActivity('Artikel abgehakt', { itemId: item.id, title: item.title, quantity: item.quantity });
}
...
async function saveDetailItem() {
    ...
    if (!(quantity > 0)) return alertUser('Bitte eine gÃ¼ltige Menge eingeben.', 'error');
    await updateDoc(doc(sub(state.listId, 'items'), item.id), { title, quantity, categoryId, note, persistentNote });
...
async function moveDetailItem() {
    ...
    const targetListId = document.getElementById('el-d-move-list')?.value || '';
    if (!targetListId) return alertUser('Bitte Ziel-Liste auswÃ¤hlen.', 'error');
    const targetList = state.lists.find((x) => x.id === targetListId);
    if (!targetList) return alertUser('Ziel-Liste wurde nicht gefunden.', 'error');
    if (!(canAdd(targetList) || canShop(targetList))) return alertUser('Keine Berechtigung fÃ¼r die Ziel-Liste.', 'error');
    if (!(await acquireLock(item.id))) return;
    const { id, ...payload } = item;
    await setDoc(doc(sub(targetListId, 'items'), id), payload);
    await deleteDoc(doc(sub(state.listId, 'items'), id));
    await updateDoc(listDoc(state.listId), { updatedAt: serverTimestamp(), updatedBy: uid(), updatedByName: uname() });
    await updateDoc(listDoc(targetListId), { updatedAt: serverTimestamp(), updatedBy: uid(), updatedByName: uname() });
    await logActivity('Artikel verschoben', { itemId: id, title: item.title, toListId: targetListId, toListName: targetList.name });
    await logActivityForList(targetListId, 'Artikel Ã¼bernommen', { itemId: id, title: item.title, fromListId: state.listId, fromListName: activeList()?.name || '' });
    state.detailId = null;
    render();
}
...
async function deleteDetailItem() {
    const item = state.items.find((x) => x.id === state.detailId);
    if (!item) return;
    if (!canEditItems()) return alertUser('Keine Berechtigung zum LÃ¶schen.', 'error');
    if (!confirm(`Produkt "${item.title}" lÃ¶schen?`)) return;
    if (!(await acquireLock(item.id))) return;
    await deleteDoc(doc(sub(state.listId, 'items'), item.id));
    await updateDoc(listDoc(state.listId), { updatedAt: serverTimestamp(), updatedBy: uid(), updatedByName: uname() });
    await logActivity('Artikel gelÃ¶scht', { itemId: item.id, title: item.title });
    state.detailId = null;
    render();
}
...
function openPurchase(itemOrArticle, isScan) {
    ...
    const unit = item?.unit || article?.defaultUnit || 'StÃ¼ck';
...
async function confirmPurchase() {
    ...
    if (p.itemId) {
        ...
        await addDoc(sub(state.listId, 'items'), { articleId: article.id, title: article.title, quantity: qty, unit: article.defaultUnit || 'StÃ¼ck', categoryId: article.categoryId || '', storeIds: article.storeIds || [], status: 'checked', note: '', persistentNote: (article.persistentNotes || []).join(' Â· '), eanCodes: article.eanCodes || [], createdAt: serverTimestamp(), createdBy: uid(), createdByName: uname(), purchasedQuantity: qty, checkedAt: serverTimestamp(), checkedBy: uid(), checkedByName: uname() });
        await logActivity('Scan Ã¼bernommen', { articleId: article.id, quantity: qty });
    }
    state.purchase = null;
    render();
}
...
async function addScannedArticleToList(article, code = '') {
    ...
    const unit = String(state.unit || article.defaultUnit || 'StÃ¼ck') || 'StÃ¼ck';
    ...
    await updateDoc(listDoc(state.listId), { updatedAt: serverTimestamp(), updatedBy: uid(), updatedByName: uname(), storeOrder: activeList()?.storeOrder?.length ? activeList().storeOrder : state.stores.map((s) => s.id) });
    await logActivity('Artikel per Scan hinzugefÃ¼gt', { articleId: article.id, title: article.title, code, quantity, unit });
    state.scanStatus = `HinzugefÃ¼gt: ${article.title}`;
    updateScannerDynamicUi();
}
...
async function deleteList(id) {
    const list = state.lists.find((x) => x.id === id); if (!list || list.ownerId !== uid() || list.isPrivateSystemList) return; if (!confirm(`Liste "${list.name}" lÃ¶schen?`)) return; await deleteDoc(listDoc(id)); if (state.listId === id) state.listId = null; persistListId(); }

async function saveCollaborator() {
    const list = activeList(); if (!list || list.ownerId !== uid() || list.isPrivateSystemList) return; const c = state.collab; if (!c.userId) return alertUser('Bitte Person auswÃ¤hlen.', 'error'); await setDoc(doc(sub(list.id, 'permissions'), c.userId), { listId: list.id, userId: c.userId, userName: USERS[c.userId]?.name || c.userId, accessFrom: fromLocal(c.accessFrom || dtLocal(new Date())), accessUntil: fromLocal(c.accessUntil || ''), canRead: !!c.canRead, canAdd: !!c.canAdd, canShop: !!c.canShop, canManage: !!c.canManage, canManageWrite: !!c.canManageWrite, paused: list.active === false, updatedAt: serverTimestamp(), updatedBy: uid(), updatedByName: uname() }); await updateDoc(listDoc(list.id), { memberIds: Array.from(new Set([...(list.memberIds || []), c.userId])), updatedAt: serverTimestamp(), updatedBy: uid(), updatedByName: uname() }); await logActivity('Mitbearbeiter gespeichert', { userId: c.userId });
}
...
async function onClick(e) {
    ...
    if (a === 'delete-article') { await deleteDoc(doc(master('articles'), btn.dataset.id)); await logActivity('Artikel gelÃ¶scht', { articleId: btn.dataset.id }); state.articleEditor = null; render(); return; }
    if (a === 'add-category') { await addMaster('categories', state.drafts.category, 'name'); state.drafts.category = ''; render(); return; }
    if (a === 'del-category') { await deleteDoc(doc(master('categories'), btn.dataset.id)); await logActivity('Kategorie gelÃ¶scht', { categoryId: btn.dataset.id }); return; }
    if (a === 'add-store') { await addMaster('stores', state.drafts.store, 'name'); state.drafts.store = ''; render(); return; }
    if (a === 'del-store-master') { await deleteDoc(doc(master('stores'), btn.dataset.id)); await logActivity('GeschÃ¤ft gelÃ¶scht', { storeId: btn.dataset.id }); return; }
