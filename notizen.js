...
function renderActiveFiltersNotizen() {
    ...
    container.innerHTML = activeFilters.map((filter) => `
        <div class="flex items-center ${filter.negate ? 'bg-red-100 text-red-800 border-red-200' : 'bg-indigo-100 text-indigo-800 border-indigo-200'} text-xs font-bold px-2 py-1 rounded-full border">
            ...
            <span>${filter.label || filter.category}: ${filter.category === 'status' ? (NOTIZ_STATUS[filter.value]?.label || filter.rawValue || filter.value) : (filter.rawValue || filter.value)}</span>
            <button onclick="window.removeNotizFilterById(${filter.id})" class="ml-1 ${filter.negate ? 'text-red-500 hover:text-red-900' : 'text-indigo-500 hover:text-indigo-900'} focus:outline-none" title="Filter entfernen">&times;</button>
        </div>
    `).join('');
}
